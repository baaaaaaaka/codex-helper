...
NATIVE_COMPAT_REPAIRABLE_VERSION_PREFIXES = ("GLIBC_", "GLIBCXX_", "CXXABI_", "GCC_")
...
def native_compat_repairable_issue(item):
    ...
    if kind == "missing_library":
        return item.get("library", "") in NATIVE_COMPAT_REPAIRABLE_LIBRARIES
    if kind == "missing_symbol_version":
        version = item.get("required_version", "")
        return version.startswith(NATIVE_COMPAT_REPAIRABLE_VERSION_PREFIXES)
    return False
