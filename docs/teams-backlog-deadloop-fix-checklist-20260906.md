...
- 2026-09-06: PR CI exposed a stale mutation-smoke source locator after the listener split ordinary and mandatory maintenance into separate switch branches. Updated the harness to target the current ordinary linked-transcript branch; `bash scripts/ci/teams_recovery_mutation_smoke.sh` passed all three baselines and killed the owner-CAS, continuation-budget, and chat-error-global mutants.
