...
| `C03-F03/F07/F08/F11/F12` | Service or worker env filtering can remove proxy, auth, provider, shell startup, or store variables needed by Graph, proxy, or beacon. | Service config has env allowlists and loopback proxy drop/keep controls; beacon provider env vars are explicit fallback, profile-stored adapter commands avoid requiring a service reload for ordinary profile registration/update, and profile/env adapter shell mode can resolve the user's configured shell environment before direct-running scheduler adapters when the submit path depends on modules, `submit_job`, NSS, or account variables. `TestBeaconProviderEnvironmentVariablesStayDocumented` verifies README, built-in skill refs, control fallback help, and this matrix list every managed Slurm/LSF provider adapter env var. | Broader service/update env-var taxonomy should still cover non-beacon env additions. |
