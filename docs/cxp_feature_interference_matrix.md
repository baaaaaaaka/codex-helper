...
| Beacon profile/target | `internal/beacon/beacon_test.go`, profile revision history/rollback/gc/archive tests, strict profile doctor adapter tests, `internal/teams/beacon_commands_test.go`, `internal/cli/beacon_cmd_test.go`, `TestPlanTurnExecutionLocalProviderReadyMachineDoesNotRequireProviderJob`. | Service-env doctor smoke still needs live scheduler coverage. |
