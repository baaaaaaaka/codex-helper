...
func TestGeneratedMachineReadinessIsolationAndHealthMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "pending provider state with accepting worker", machine: func(m *Machine) { m.ProviderState = ProviderJobPending }, want: true},
		{name: "suspended provider state", machine: func(m *Machine) { m.ProviderState = ProviderJobSuspended }},
