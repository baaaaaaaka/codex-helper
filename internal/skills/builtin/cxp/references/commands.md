...
- If worker doctor reports `missing codex`, set scheduler PATH or pass worker `--codex-path <codex-or-wrapper>` from the adapter. A wrapper is the right place to add Codex exec flags such as `--skip-git-repo-check`; Teams service `--codex-arg` settings do not automatically reach remote beacon workers. The generated templates honor `CXP_BEACON_CODEX_BIN` by passing it as worker `--codex-path`.
