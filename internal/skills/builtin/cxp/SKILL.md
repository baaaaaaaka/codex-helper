---
name: cxp
description: "Use when a user asks about cxp, codex-proxy, or codex-helper operations: proxy/SSH profiles, model profiles/Responses adapter/YOLO mode, Teams helper/control/work chats, Codex history or skills, upgrades, beacon/execution target/profile switching, Slurm/LSF/GPU/local execution, and safe handoffs for operations that can interrupt Codex."
---
...
Trigger on related requests even when the user does not say `cxp`: model profiles, Responses adapter, YOLO mode, beacon mode, execution target/profile switching, GPU/Slurm/LSF/local execution, Teams helper/control/work chats, proxy/SSH profiles, history, skills, and upgrades.
