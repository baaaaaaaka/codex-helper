...
type Registry struct {
	...
	MachineHostnameOverride string               `json:"machine_hostname_override,omitempty"`
...
func mergeRegistrySessions(existing []Session, next []Session) []Session {
	...
	for _, session := range next {
		...
		if existingSession, ok := existingByID[id]; ok {
			...
			if strings.TrimSpace(session.CodexThreadID) == "" && session.ModelGeneration == existingSession.ModelGeneration {
