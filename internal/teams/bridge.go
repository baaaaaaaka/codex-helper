...
const (
	...
	// Ambiguous-send reconciliation is a cold path.  Follow enough same-origin
	...
	// converted into a permanent history checkpoint block that users cannot
	// recover from during ordinary polling.
	transcriptSyncMaxRecordsPerSessionPerCycle = 8
	liveTranscriptBackfillMaxRecords           = 3
	// Kept as a fixture-size constant for compatibility with historical tests;
	// it is no longer an automatic blocking threshold.
	...
	// the byte budget; otherwise one large but valid answer could starve the
	// queue forever.
...
var errTranscriptAutomaticSourceProofUnavailable = errors.New("automatic transcript source proof is unavailable")
...
const (
	...
	recoveryReasonAmbiguousCodexExecutionPrefix         = "ambiguous Codex execution:"
	recoveryReasonHelperContextCanceledUnverified       = "helper context canceled before Codex result could be verified"
	recoveryReasonCodexExecutionConfirmed               = "Codex execution ownership confirmed"
)
...
type outboxQueueOptions struct {
	...
	// not use this proof because it establishes a new operator-directed cursor.
	ExpectedSourcePath            string
	ExpectedSourceFingerprint     string
	ExpectedSourceOffset          int64
	ExpectedSourceOffsetKnown     bool
	ExpectedSourceReadFingerprint string
	ExpectedSourceReadStartOffset int64
	ExpectedSourceReadEndOffset   int64
	ExpectedSourceReadRangeKnown  bool
}
...
type durableSessionProjectionVersion struct {
	ChatID            string
...
// NewBridgeForChatRecreateWithHTTPClient pins the existing authoritative store
// before a recreate command drains the running listener. Unlike normal startup
// resolution, this path deliberately does not migrate the selected store.
func NewBridgeForChatRecreateWithHTTPClient(ctx context.Context, auth *AuthManager, registryPath string, out io.Writer, client *http.Client) (*Bridge, error) {
	graph := NewGraphClientWithHTTPClient(auth, out, client)
	readGraph, err := NewReadGraphClientWithHTTPClient(out, client)
	if err != nil {
		return nil, err
	...
	scope, storePath, err := ResolveStorePathForMaintenance(ScopeIdentityForUser(user))
	if err != nil {
		return nil, err
	}
...
func newBridgeForPinnedMaintenanceStore(graph *GraphClient, readGraph *GraphClient, registryPath string, out io.Writer, user User, scope teamstore.ScopeIdentity, storePath string) (*Bridge, error) {
	...
	if strings.TrimSpace(registryPath) == "" {
		...
		if samePath(storePath, currentStorePath) {
			registryPath, err = appdirs.StatePath("teams", "scopes", safeScopePathPart(scope.ID), "registry.json")
		} else {
			registryPath, err = registryPathForResolvedScopeStore(scope.ID, storePath)
		}
		if err != nil {
			return nil, err
		}
...
func (b *Bridge) Listen(ctx context.Context, opts BridgeOptions) error {
	...
	if retired, err := b.retireLegacyHistoryGateOutbox(ctx); err != nil {
		...
		if b.out != nil {
			_, _ = fmt.Fprintf(b.out, "Teams legacy history-gate outbox cleanup skipped: %v\n", err)
		}
...
func (b *Bridge) syncRegistrySessionProjection(session Session) {
	...
	if b.durableProjectionVersionBySession == nil {
		b.durableProjectionVersionBySession = make(map[string]durableSessionProjectionVersion)
	}
	b.durableProjectionVersionBySession[session.ID] = version
	b.reg.ensureMaps()
	if current := b.reg.SessionByID(session.ID); current != nil {
		if registrySessionsEqual(*current, session) {
			return
		}
		oldChatID := strings.TrimSpace(current.ChatID)
		*current = session
		if oldChatID != "" && oldChatID != session.ChatID {
			delete(b.reg.Chats, oldChatID)
		}
		if _, ok := b.reg.Chats[session.ChatID]; !ok {
			b.reg.Chats[session.ChatID] = ChatState{}
		}
		b.registryProjectionDirty = true
		return
	}
...
func (b *Bridge) recoverIdleWorkChatContinuationBeforePark(ctx context.Context, state teamstore.State, session Session, poll teamstore.ChatPollState) (bool, error) {
	...
	// message handler. Heal the projection first so those pre-handler stages do
	// not observe an old chat binding during auto-park recovery.
	b.syncRegistrySessionProjection(session)
	...
	return b.pollChatWithRoleStateOptions(ctx, session.ChatID, effectiveOwnerPollTop(ownerPollMessageTop), inboundPollRoleWork, false, poll, true, pollChatWithRoleOptions{
		...
		return b.handleResolvedSessionMessageWithQueueState(ctx, &session, session.ChatID, msg, text, &turns, &state)
	})
}
...
func durableAutoParkHandoffScheduleUpdate(decision inboundPollDecision) (teamstore.ChatPollScheduleUpdate, bool) {
	if !decision.ShouldPark || strings.TrimSpace(decision.ChatID) == "" {
		return teamstore.ChatPollScheduleUpdate{}, false
	}
	return teamstore.ChatPollScheduleUpdate{
		ChatID:            decision.ChatID,
...
func controlAdvancedHelpText() string {
	return strings.Join([]string{
		...
		"",
		"Local CLI:",
		"- `cxp skills install-builtin` - install or repair bundled local skills in `$HOME/.agents/skills`",
...
func (b *Bridge) handleSessionMessageWithQueueState(ctx context.Context, chatID string, msg ChatMessage, text string, knownTurns *sessionTurnQueueState, knownQueueSnapshot *teamstore.State) error {
	return b.handleResolvedSessionMessageWithQueueState(ctx, nil, chatID, msg, text, knownTurns, knownQueueSnapshot)
}
...
func (b *Bridge) handleResolvedSessionMessageWithQueueState(ctx context.Context, session *Session, chatID string, msg ChatMessage, text string, knownTurns *sessionTurnQueueState, knownQueueSnapshot *teamstore.State) error {
	...
	if session == nil {
		return fmt.Errorf("Teams work session not found for chat %q", strings.TrimSpace(chatID))
	}
	if strings.TrimSpace(session.ChatID) == "" || strings.TrimSpace(session.ChatID) != strings.TrimSpace(chatID) {
		return fmt.Errorf("Teams work session %q binding does not match chat %q", session.ID, strings.TrimSpace(chatID))
	}
	if isPromptlessTeamsAttachmentPlaceholderMessage(msg) {
		b.markRegistrySeen(chatID, msg.ID)
		return nil
	...
	if b.asyncTurns {
		...
		gate, err := b.prepareLocalCodexBeforeTeamsTurnWithQueueState(ctx, session, knownTurns)
		if err != nil {
			return err
		}
...
func (b *Bridge) retryTurnCommand(ctx context.Context, session *Session, turnID string) error {
	...
	if unresolved, err := b.sessionExecutionOwnershipUnresolved(ctx, *session); err != nil {
		...
		return b.sendToChat(ctx, session.ChatID, "Retry is paused because a previous Codex execution is still unconfirmed. I will not retry automatically or start another same-thread turn. Check recent work, then send `helper publish-history` if you want to publish pending history.")
	}
	state, err := b.store.Load(ctx)
	if err != nil {
		return err
	...
	if strings.TrimSpace(turn.RecoveryReason) == recoveryReasonCodexExecutionConfirmed {
		return b.sendToChat(ctx, session.ChatID, "turn "+turn.ID+" already has confirmed Codex ownership and cannot be retried; retrying it could duplicate work.")
	}
	switch turn.Status {
...
func (b *Bridge) queueTeamsPromptAckWithBodyForMessage(ctx context.Context, session *Session, turn teamstore.Turn, body string, msg ChatMessage) error {
	...
	if err := b.sendQueuedOutboxWithOptions(ctx, queued, outboxSendOptions{RespectRateLimitBlock: true, RecordRateLimit: true, AllowProtectedAmbiguousBypass: true}); err != nil && b.out != nil {
...
func (b *Bridge) processQueuedTurnsWithStartBudget(ctx context.Context, startLimit int, enforceStartLimit bool) (int, error) {
	...
	knownTurnsBySession := make(map[string]sessionTurnQueueState)
	for _, turn := range state.Turns {
		if strings.TrimSpace(turn.SessionID) == "" {
			continue
		}
		knownTurns := knownTurnsBySession[turn.SessionID]
		switch turn.Status {
		case teamstore.TurnStatusRunning:
			knownTurns.Running = true
		case teamstore.TurnStatusQueued:
		...
		knownTurnsBySession[turn.SessionID] = knownTurns
	}
	var sessionIDs []string
	for sessionID, knownTurns := range knownTurnsBySession {
		if knownTurns.Queued > 0 && !knownTurns.Running {
	...
	for _, sessionID := range sessionIDs {
		...
		knownTurns := knownTurnsBySession[sessionID]
		gate, err := b.prepareLocalCodexBeforeTeamsTurnWithQueueState(ctx, session, &knownTurns)
		if err != nil {
...
func (b *Bridge) processQueuedTurnsForSession(ctx context.Context, session *Session) error {
	...
	knownTurns := sessionTurnQueueStateFromSnapshot(state, session.ID)
	if knownTurns.Running {
		return nil
	}
	...
	if !ok {
		if unresolved, err := b.sessionExecutionOwnershipUnresolved(ctx, *session); err != nil {
			...
			// Do not apply a pending model/profile generation while the old
			// app-server execution is unresolved. Switching generation can close
			// the only runner that still owns the cancellation fence.
			return nil
		}
...
func (b *Bridge) sendQueuedTurnAttentionIfDue(ctx context.Context, session *Session, turn teamstore.Turn, gate localCodexBeforeTeamsGate, now time.Time) error {
	...
	// Flush only this session/turn. A chat-wide flush here could release older
	// transcript answer outbox messages after an unresolved anchor was just
	// detected, defeating the fail-closed barrier. Waiting notices are
	// best-effort, so preserve the old error-isolation behavior.
	if err := b.sendQueuedOutboxWithOptions(ctx, queued, outboxSendOptions{
...
func queuedTurnBlockedReason(gate localCodexBeforeTeamsGate) string {
	...
	case strings.Contains(text, "previous Codex execution is still unconfirmed"):
		return "A previous Codex execution is still unconfirmed; I will not retry or start another same-thread turn."
	default:
...
func (b *Bridge) sessionForInboundEvent(ctx context.Context, inbound teamstore.InboundEvent) (*Session, error) {
	if b == nil {
		return nil, nil
	}
	...
	if b.store != nil && sessionID != "" {
		...
		if _, ok := sessions[sessionID]; ok {
			return b.sessionForIDState(teamstore.State{Sessions: sessions}, sessionID), nil
		}
		hasSessions, err := b.store.HasSessions(ctx)
		if err != nil {
			return nil, err
	...
	if chatID := strings.TrimSpace(inbound.TeamsChatID); chatID != "" {
		return b.reg.SessionByChatID(chatID), nil
	}
	return nil, nil
}
...
func (b *Bridge) sessionForIDState(state teamstore.State, sessionID string) *Session {
	...
	if ok && strings.TrimSpace(durable.TeamsChatID) != "" {
		session := registrySessionFromDurable(durable)
		if strings.TrimSpace(session.ID) == "" {
...
func (b *Bridge) recoverQueuedTurn(ctx context.Context, session *Session, turn teamstore.Turn, state teamstore.State) error {
	inbound, ok := state.InboundEvents[turn.InboundEventID]
	if !ok || inbound.TeamsMessageID == "" {
		marked, err := b.markTurnInterruptedUnlessTerminal(ctx, turn.ID, "queued turn missing original Teams message")
		if err != nil {
			return err
		}
		if !marked {
...
func (b *Bridge) cancelTurnCommandForScope(ctx context.Context, session *Session, turnID string, scopeLabel string) error {
	...
	switch turn.Status {
	case teamstore.TurnStatusQueued:
		marked, err := b.markTurnInterruptedUnlessTerminal(ctx, turn.ID, "canceled by user")
		if err != nil {
			return err
		}
		if !marked {
...
func (b *Bridge) runQueuedTurnInputWithExecutor(ctx context.Context, executor Executor, session *Session, turn teamstore.Turn, chatID string, input ExecutionInput) error {
	...
	// A queued request canceled before this runner acquired ownership has not
	// reached Codex at all. Leave it queued for the normal cancellation/queue
	// policy; do not turn a pre-dispatch context error into an interrupted turn.
	if turn.Status == teamstore.TurnStatusQueued && ctx != nil && ctx.Err() != nil {
		return nil
	}
	sessionID := ""
	if session != nil {
	...
	if turn.Status == teamstore.TurnStatusRunning {
		if unresolved, ownershipErr := b.liveTurnExecutionOwnershipUnresolved(ctx, *session, turn); ownershipErr != nil {
			...
			if requeued.Status != teamstore.TurnStatusQueued {
				return nil
			}
	...
	// Synchronous callers can invoke this path without ClaimNextQueuedTurn.
	...
	if err != nil {
		if errors.Is(err, teamstore.ErrUnresolvedExecution) {
			...
			if requeued.Status == teamstore.TurnStatusQueued {
				...
				return b.sendQueuedTurnAttentionIfDue(requeueCtx, session, requeued, gate, time.Now())
			}
			return nil
		}
		return err
	...
	turn = startedTurn
	// The durable mark is the first atomic fence.  Recheck immediately after
	...
	if unresolved, ownershipErr := b.liveTurnExecutionOwnershipUnresolved(ctx, *session, turn); ownershipErr != nil {
		...
		if b.out != nil {
			_, _ = fmt.Fprintf(b.out, "Teams execution ownership recheck deferred dispatch: %v\n", ownershipErr)
		}
		return ownershipErr
	} else if unresolved {
		requeueCtx := ctx
		...
		if requeued.Status == teamstore.TurnStatusQueued {
			...
			return b.sendQueuedTurnAttentionIfDue(ctx, session, requeued, gate, time.Now())
		}
		return nil
	}
	if executor == nil {
	...
	if err != nil {
		...
		if isUnresolvedExecutionFenceError(err) {
			notifyCtx := ctx
			if notifyCtx == nil || notifyCtx.Err() != nil {
				notifyCtx = context.Background()
			}
			anchorTurn := turn
			anchorTurn.CodexThreadID = firstNonEmptyString(result.CodexThreadID, anchorTurn.CodexThreadID, session.CodexThreadID)
			anchorTurn.CodexTurnID = firstNonEmptyString(result.CodexTurnID, anchorTurn.CodexTurnID)
			anchorTurn.RecoveryReason = recoveryReasonAmbiguousCodexExecutionPrefix + " cancellation fence unconfirmed"
			// Persist the anchor and interrupted status in one store transaction;
			// a terminal callback can therefore win the race without leaving a
			// Completed turn fenced by a newly-created anchor.
			persisted, anchorErr := b.persistUnresolvedExecutionAnchorForTurn(notifyCtx, *session, anchorTurn)
			if anchorErr != nil {
				return anchorErr
			}
			...
			body := "Codex did not confirm that the previous same-thread cancellation finished. I did not start or retry another request, and I did not publish a possibly duplicated answer. Check the existing Codex work; normal dispatch resumes only after the cancellation fence is explicitly confirmed."
			if detail := strings.TrimSpace(trimTeamsCommandOutput(err.Error(), 600)); detail != "" {
				body += "\n\nDiagnostic for the admin:\n" + detail
			}
			return b.queueAndSendOutboxChunksWithOptions(notifyCtx, session.ID, turn.ID, chatID, "interrupted", body, outboxQueueOptions{
				MentionOwner:        true,
				NotificationKind:    "needs_attention",
				IgnoreEarlierOutbox: true,
			})
		...
		if cancelRequested && isCanceledExecutionError(err) {
			...
			// A user cancel can race the app-server's terminal response. Prefer a
			// provenance-matched final already written to the transcript; treating
			// that response as canceled would create a second retry-looking answer.
			if recovered, ok := b.completedTurnResultAfterExecutionError(recoveryCtx, session, turn, result); ok {
				return b.completeQueuedTurnWithResult(recoveryCtx, session, turn, chatID, plan, recovered)
			}
			if plan.Action == beacon.TurnRunBeacon || plan.Action == beacon.TurnWaitAllocation {
				if beaconErr := b.cancelBeaconTurn(recoveryCtx, session, turn, firstNonEmptyString(cancelReason, "canceled by user")); beaconErr != nil && b.out != nil {
			...
			return b.queueAndSendOutboxChunks(recoveryCtx, session.ID, turn.ID, chatID, "canceled", "Codex request canceled.")
		}
		...
		if isCanceledExecutionError(err) {
			...
			reason := recoveryReasonHelperContextCanceledUnverified
			anchorTurn := turn
			anchorTurn.CodexThreadID = firstNonEmptyString(result.CodexThreadID, anchorTurn.CodexThreadID, session.CodexThreadID)
			anchorTurn.CodexTurnID = firstNonEmptyString(result.CodexTurnID, anchorTurn.CodexTurnID)
			anchorTurn.RecoveryReason = reason
			persisted, anchorErr := b.persistUnresolvedExecutionAnchorForTurn(notifyCtx, *session, anchorTurn)
			if anchorErr != nil {
				return anchorErr
			}
		...
		if IsAmbiguousExecutionError(err) {
			...
			if blocked, bindErr := b.bindObservedCodexThreadOrInterrupt(recoveryCtx, session, turn, result.CodexThreadID, "runner_ambiguous"); bindErr != nil {
				return bindErr
			} else if blocked {
				return nil
			}
			...
			reason := recoveryReasonAmbiguousCodexExecutionPrefix + " " + err.Error()
			anchorTurn := turn
			anchorTurn.CodexThreadID = firstNonEmptyString(result.CodexThreadID, anchorTurn.CodexThreadID, session.CodexThreadID)
			anchorTurn.CodexTurnID = firstNonEmptyString(result.CodexTurnID, anchorTurn.CodexTurnID)
			anchorTurn.RecoveryReason = reason
			persisted, anchorErr := b.persistUnresolvedExecutionAnchorForTurn(recoveryCtx, *session, anchorTurn)
			if anchorErr != nil {
				return anchorErr
			}
			if persisted.Terminal {
			...
			body := "Codex accepted the request, but the helper could not confirm whether it finished. I did not retry automatically because that could duplicate work.\n\nCheck recent messages and changed files first. I will not retry while execution ownership is unresolved; after the fence is confirmed, you may use `helper retry last` (or `helper retry " + turn.ID + "`)."
			if detail := strings.TrimSpace(trimTeamsCommandOutput(err.Error(), 600)); detail != "" {
				body += "\n\nDiagnostic for the admin:\n" + detail
			}
			return b.queueAndSendOutboxChunksWithOptions(recoveryCtx, session.ID, turn.ID, chatID, "interrupted", body, outboxQueueOptions{
				MentionOwner:        true,
				NotificationKind:    "needs_attention",
				IgnoreEarlierOutbox: true,
			})
		...
		// An unresolved anchor makes generation part of the callback proof.  Read
		// only the scoped checkpoint here as an optimistic hint; the store rereads
		// and validates it in the same failure transaction.  A missing anchor keeps
		// the legacy no-anchor path compatible.
		failureAnchorGeneration := int64(0)
		if checkpoint, found, checkpointErr := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(session.ID)); checkpointErr != nil {
			return checkpointErr
		} else if found && executionAnchorActive(checkpoint.UnresolvedExecution) {
			failureAnchorGeneration = checkpoint.UnresolvedExecution.Generation
		}
		// Resolve the callback identity and any active anchor in one store
		// transaction.  A bridge-side anchor snapshot would leave a TOCTOU
		// window in which a newly-created anchor could be missed or cleared by
		// the wrong failure callback.
		var markErr error
		if strings.TrimSpace(result.CodexThreadID) == "" && strings.TrimSpace(result.CodexTurnID) == "" {
			// A runner/infrastructure failure that never produced an app-server
	...
	if cancelRequested {
		...
		// Cancellation may be observed just after app-server returned a normal
		// terminal result.  Reconcile the canonical transcript before marking the
		// durable turn interrupted; otherwise a successful answer can be followed
		// by a misleading cancellation notice and a later retry.
		if result.canonicalTranscriptFinal || (strings.TrimSpace(result.Text) != "" && strings.TrimSpace(result.CodexTurnID) != "") {
			return b.completeQueuedTurnWithResult(recoveryCtx, session, turn, chatID, plan, result)
		}
		if recovered, ok := b.completedTurnResultAfterExecutionError(recoveryCtx, session, turn, result); ok {
			return b.completeQueuedTurnWithResult(recoveryCtx, session, turn, chatID, plan, recovered)
		}
...
func (b *Bridge) completeQueuedTurnWithResult(ctx context.Context, session *Session, turn teamstore.Turn, chatID string, plan beacon.TurnExecutionPlan, result ExecutionResult) error {
	// A completion callback can arrive after a concurrent failure/cancellation
	// callback has committed the same durable turn.  Do not queue another final
	// answer from that stale in-memory result; the store-level terminal CAS below
	// is the final defense, while this read avoids doing any outbox work at all
	// in the common already-terminal case.
	resolveInterrupted := false
	if b != nil && b.store != nil && strings.TrimSpace(turn.ID) != "" {
		current, found, err := b.store.TurnByID(ctx, turn.ID)
		if err != nil {
			return err
		}
		if !found {
			return fmt.Errorf("turn %q not found", turn.ID)
		}
		if current.Status == teamstore.TurnStatusCompleted || current.Status == teamstore.TurnStatusFailed {
			return nil
		}
		if current.Status == teamstore.TurnStatusInterrupted {
			// Interrupted completion is a separate, proof-bearing recovery path.
			// An empty or partial callback must not promote the turn merely because
			// the bridge still has an old result in memory.
			if strings.TrimSpace(result.CodexThreadID) == "" || strings.TrimSpace(result.CodexTurnID) == "" {
				return nil
			}
	...
	if !resolveInterrupted && !result.canonicalTranscriptFinal {
	...
	if resolveInterrupted {
		// Never silently replace the callback IDs with a durable binding for an
		// interrupted owner; those IDs are the recovery proof.
		if strings.TrimSpace(turn.CodexThreadID) != "" && strings.TrimSpace(turn.CodexThreadID) != strings.TrimSpace(result.CodexThreadID) ||
			strings.TrimSpace(turn.CodexTurnID) != "" && strings.TrimSpace(turn.CodexTurnID) != strings.TrimSpace(result.CodexTurnID) {
			return teamstore.ErrStaleExecutionCallback
		}
	}
	transcriptTurn := turn
	completionThreadID := firstNonEmptyString(result.CodexThreadID, turn.CodexThreadID, session.CodexThreadID)
	transcriptTurn.CodexThreadID = completionThreadID
	transcriptTurn.CodexTurnID = firstNonEmptyString(result.CodexTurnID, turn.CodexTurnID)
	completionCodexTurnID := strings.TrimSpace(result.CodexTurnID)
	if completionCodexTurnID == "" {
		// A durable owner binding is a compatibility fallback for older normal
		// adapters, but never for an unresolved anchor: an empty callback ID is
		// not proof that the callback belongs to the fenced execution.
		checkpoint, found, checkpointErr := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(session.ID))
		if checkpointErr != nil {
			return checkpointErr
		}
		if !found || !executionAnchorActive(checkpoint.UnresolvedExecution) {
			completionCodexTurnID = strings.TrimSpace(turn.CodexTurnID)
		}
	}
	// Do not create any protected final/progress outbox rows until the callback
	// has passed a non-mutating ownership preflight.  The final outbox and the
	// terminal Turn/checkpoint CAS are committed together by the store below;
	// this preflight is only an early rejection optimization.
	// A missing thread ID may use the durable session/thread binding for legacy
	// adapters; the Codex turn ID is never synthesized.  If an unresolved
	// anchor exists, the store still requires both exact IDs and rejects this
	// callback before it can create a final.
	if !resolveInterrupted {
		if err := b.store.ValidateTurnCompletionOwnership(ctx, turn.ID, completionThreadID, completionCodexTurnID); err != nil {
			return err
		}
	}
	...
	// Capture the generation as an optimistic proof.  The store rereads the
	// checkpoint inside its transaction and rejects a recreated anchor.
	anchorGeneration := int64(0)
	anchorSourceFingerprint := ""
	if checkpoint, found, proofErr := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(session.ID)); proofErr == nil && found && executionAnchorActive(checkpoint.UnresolvedExecution) {
		anchorGeneration = checkpoint.UnresolvedExecution.Generation
		anchorSourceFingerprint = strings.TrimSpace(checkpoint.UnresolvedExecution.SourceFingerprint)
	}
	...
	if preFinal.HasFinalCheckpoint() {
		...
		progress = teamstore.TranscriptCheckpointProgress{
			...
			LastSourceLine:          preFinal.FinalCheckpoint.SourceLine,
			LastOffset:              preFinal.FinalCheckpoint.SourceOffset,
	...
	if err != nil {
		if errors.Is(err, teamstore.ErrUnresolvedExecution) {
			// Legacy rows from an earlier attempt are still quarantined here.  The
			// current callback cannot create a new final until the ownership CAS
			// succeeds.
			if _, quarantineErr := b.store.QuarantineQueuedTerminalAnswerOutbox(ctx, turn.ID, "superseded by unresolved execution"); quarantineErr != nil && b.out != nil {
				_, _ = fmt.Fprintf(b.out, "Teams unresolved completion final quarantine failed: %v\n", quarantineErr)
			}
		}
		if errors.Is(err, teamstore.ErrCompletionOwnerLost) {
			// Another terminal callback won.  Its durable result and any final it
			// owns are authoritative; this stale callback must not notify or retry.
			return nil
		}
		return err
	}
	...
	if preFinal.Queued > 0 || len(planned) > 0 {
...
func (b *Bridge) completedTurnResultFromLinkedTranscript(ctx context.Context, session *Session, turn teamstore.Turn, observed ExecutionResult) (ExecutionResult, bool) {
	...
	// Completion recovery reads the transcript outside the normal linked-sync
	// scanner.  It must therefore perform the same source-prefix proof before
	// attributing a final to the interrupted turn; otherwise a replaced file
	// could be scanned from the old cursor and committed by the terminal CAS.
	if strings.TrimSpace(checkpoint.SourcePath) == "" ||
		cleanComparablePath(checkpoint.SourcePath) != cleanComparablePath(local.FilePath) ||
		strings.TrimSpace(checkpoint.SourceFingerprint) == "" ||
		!linkedCheckpointOffsetKnown(checkpoint) ||
		!linkedCheckpointPrefixMatches(local.FilePath, checkpoint) {
		return ExecutionResult{}, false
	}
	...
	// The recovery scanner is separate from readLinkedTranscriptDelta. Recheck
	// the trusted prefix after it returns so a replacement during the scan does
	// not become a terminal answer attributed to the old checkpoint.
...
func (b *Bridge) completedTurnResultFromLocalCodexHistorySince(ctx context.Context, session *Session, turn teamstore.Turn, observed ExecutionResult, local codexhistory.Session, previous historyTieredFileState) (ExecutionResult, bool) {
	...
	// A recovered transcript final is terminal output, not a generic history
	// hint.  Without an explicit app-server turn ID there is no ownership proof
	// that a recent child/follow-up final belongs to this callback, so leave the
	// turn unresolved rather than attributing the newest visible answer.
	if observedTurnID == "" {
		return ExecutionResult{}, false
	}
	...
	if scan.TooLarge {
		// Completion recovery is a polling hot path. Never turn a large live
		// tail into an unbounded scan; use the live runner result or an explicit
		// history import instead of guessing ownership from the whole file.
		return ExecutionResult{}, false
	}
	if scan.Incomplete || scan.Truncated {
		return ExecutionResult{}, false
	}
	if scan.State.UnresolvedContinuation {
		// A persistent goal/task continuation after a prior final is not
		// ownership evidence, even when the child happens to reuse the outer
		// Codex turn ID. Refuse to recover a final from that tail.
		return ExecutionResult{}, false
	}
	...
	for _, final := range scan.Finals {
		...
		if !final.Record.TurnIDExplicit || strings.TrimSpace(final.Record.TurnID) != observedTurnID {
			continue
		}
		if !threshold.IsZero() {
...
func (b *Bridge) interruptTurnForAttachmentMessage(ctx context.Context, session *Session, turn teamstore.Turn, message string) error {
	marked, err := b.markTurnInterruptedUnlessTerminal(ctx, turn.ID, message)
	if err != nil {
		return err
	}
	if !marked {
...
// markTurnInterruptedUnlessTerminal makes stale executor/cancellation
// callbacks harmless after another owner has committed a terminal result.
// The store performs the status check atomically; this helper additionally
// lets callers suppress a misleading second notification when the callback
// loses that race.
func (b *Bridge) markTurnInterruptedUnlessTerminal(ctx context.Context, turnID string, reason string) (bool, error) {
	turn, err := b.store.MarkTurnInterrupted(ctx, turnID, reason)
	if err != nil {
		return false, err
	}
...
func sessionTurnQueueStateFromSnapshot(state teamstore.State, sessionID string) sessionTurnQueueState {
	sessionID = strings.TrimSpace(sessionID)
...
func (b *Bridge) sessionTurnQueueState(ctx context.Context, sessionID string) (sessionTurnQueueState, error) {
	...
	return sessionTurnQueueStateFromSnapshot(state, sessionID), nil
}
...
func (b *Bridge) handleClaimedQueuedTurnError(ctx context.Context, session *Session, turn teamstore.Turn, err error) {
	...
	if errors.Is(err, teamstore.ErrUnresolvedExecution) {
		b.handleUnresolvedQueuedTurnError(ctx, session, turn, err)
		return
	}
	if errors.Is(err, teamstore.ErrStaleExecutionCallback) {
		// A callback from an older/different app-server owner must not turn the
		// current owner into Interrupted through the generic queued-turn error
		// path. Keep the owner untouched, but leave one stable attention record so
		// an operator can distinguish a rejected callback from a lost response.
		if session == nil || strings.TrimSpace(session.ChatID) == "" {
			return
		}
		if queueErr := b.queueAndSendOutbox(ctx, teamstore.OutboxMessage{
			ID:               "outbox:" + strings.TrimSpace(turn.ID) + ":stale-execution-callback",
			SessionID:        strings.TrimSpace(turn.SessionID),
			TurnID:           strings.TrimSpace(turn.ID),
			TeamsChatID:      strings.TrimSpace(session.ChatID),
			Kind:             "interrupted",
			Body:             "Codex returned a stale or incomplete execution callback. I did not change the current turn or retry automatically; verify the recent Codex execution before sending another request.",
			MentionOwner:     true,
			NotificationKind: "needs_attention",
		}); queueErr != nil && b.out != nil {
			_, _ = fmt.Fprintf(b.out, "Teams stale execution callback notice failed: %v\n", queueErr)
		}
		return
	}
	if current, ok, loadErr := b.store.TurnByID(ctx, turn.ID); loadErr == nil && ok {
		switch current.Status {
		case teamstore.TurnStatusQueued, teamstore.TurnStatusCompleted, teamstore.TurnStatusFailed, teamstore.TurnStatusInterrupted:
			// A failed ownership recheck may have deliberately requeued the
			// claimed turn. Never convert that safe queued state into an
			// interrupted turn merely because the stale caller returned an error.
			return
		}
	}
...
// handleUnresolvedQueuedTurnError is deliberately separate from the ordinary
// queued-turn failure path.  Ownership ambiguity is not a retryable execution
// error: protected final rows must be quarantined, the anchor must remain
// unresolved, and only one stable needs_attention notice may be published.
func (b *Bridge) handleUnresolvedQueuedTurnError(ctx context.Context, session *Session, turn teamstore.Turn, cause error) {
	if b == nil || b.store == nil {
		return
	}
	reason := recoveryReasonAmbiguousCodexExecutionPrefix + " completion ownership mismatch"
	if _, quarantineErr := b.store.QuarantineQueuedTerminalAnswerOutbox(ctx, turn.ID, "superseded by unresolved execution"); quarantineErr != nil && b.out != nil {
		_, _ = fmt.Fprintf(b.out, "Teams unresolved completion final quarantine failed: %v\n", quarantineErr)
	}
	effectiveSession := Session{ID: strings.TrimSpace(turn.SessionID)}
	if session != nil {
		effectiveSession = *session
	}
	anchorTurn := turn
	anchorTurn.RecoveryReason = reason
	persisted, anchorErr := b.persistUnresolvedExecutionAnchorForTurn(ctx, effectiveSession, anchorTurn)
	if anchorErr != nil {
		if b.out != nil {
			_, _ = fmt.Fprintf(b.out, "Teams unresolved completion anchor persistence failed: %v\n", anchorErr)
		}
		return
	}
	if persisted.Terminal {
	...
	detail := "Codex execution ownership is still unresolved. I did not publish or retry this final."
	if cause != nil && strings.TrimSpace(cause.Error()) != "" {
		detail += "\n\nDiagnostic: " + strings.TrimSpace(cause.Error())
	}
	if queueErr := b.queueAndSendOutbox(ctx, teamstore.OutboxMessage{
		ID:               "outbox:" + turn.ID + ":queued-turn-unresolved",
		SessionID:        turn.SessionID,
		TurnID:           turn.ID,
		TeamsChatID:      chatID,
		Kind:             "interrupted",
		...
		_, _ = fmt.Fprintf(b.out, "Teams unresolved queued turn notice failed: %v\n", queueErr)
	}
}
...
func (b *Bridge) runExecutorWithHeartbeat(ctx context.Context, executor Executor, session *Session, turn teamstore.Turn, chatID string, input ExecutionInput) (ExecutionResult, error) {
	...
	if err := b.recordOwnerHeartbeat(ctx, session.ID, turn.ID); err != nil {
		if ctx.Err() != nil {
			return ExecutionResult{}, fmt.Errorf("%w: %v", errExecutorNotDispatched, ctx.Err())
		}
...
// PinStoreForMaintenance resolves and opens the authoritative store while the
// running listener's owner metadata is still available. Maintenance commands
// that drain the listener must call this before draining so a later path
// resolution cannot switch to a retained migration mirror.
func (b *Bridge) PinStoreForMaintenance() (string, error) {
	if b == nil {
...
// retryKnownAmbiguousOutbox is intentionally narrower than the normal outbox
// flusher. It is used when an idempotent command queues the same deterministic
// row again in the same Bridge that just observed an ambiguous Graph result.
// The in-memory attempt claim prevents a concurrent sender from being robbed;
// a restarted helper must wait for normal lease recovery or an explicit Graph
// reconciliation instead of guessing that the old POST failed.
func (b *Bridge) retryKnownAmbiguousOutbox(ctx context.Context, outbox teamstore.OutboxMessage) (bool, error) {
	if b == nil || outbox.Status != teamstore.OutboxStatusSending || !teamstore.OutboxSendIsAmbiguous(outbox) {
		return false, nil
	}
	generation, ok := b.claimKnownAmbiguousOutboxRetry(outbox.ID)
	if !ok {
		return false, nil
	}
	defer b.releaseKnownAmbiguousOutboxRetry(outbox.ID, generation)
	err := b.sendQueuedOutboxWithOptions(ctx, outbox, outboxSendOptions{
		RespectRateLimitBlock:    true,
		RecordRateLimit:          true,
...
func (b *Bridge) queueOrSendOutboxChunks(ctx context.Context, sessionID string, turnID string, chatID string, kind string, text string, opts outboxQueueOptions, queueOnly bool) error {
	...
	for _, msg := range queued {
		if retried, err := b.retryKnownAmbiguousOutbox(ctx, msg); retried {
			if err != nil {
				return err
			}
		}
	}
	flushOpts := outboxFlushOptions{
		AllowAmbiguousRetry:           outboxTurnAllowsAmbiguousRetry(turnID),
		AllowProtectedAmbiguousBypass: outboxTurnAllowsProtectedAmbiguousBypass(turnID),
		SkipUnresolvedTranscript:      true,
	}
	if opts.IgnoreEarlierOutbox {
		flushOpts.IgnoreEarlierOutbox = true
		flushOpts.SkipUnresolvedTranscript = true
	}
	if err := b.flushPendingOutboxFilteredWithOptions(ctx, "", "", chatID, flushOpts); err != nil {
		return err
	}
	b.boostPolling(time.Now())
	return nil
...
// planOutboxChunksWithOptions renders a deterministic outbox plan without
...
func (b *Bridge) planOutboxChunksWithOptions(sessionID string, turnID string, chatID string, kind string, text string, opts outboxQueueOptions) ([]teamstore.OutboxMessage, error) {
	...
	for i, chunk := range chunks {
		...
		if strings.TrimSpace(opts.ExpectedSourceFingerprint) != "" &&
			strings.TrimSpace(opts.ExpectedSourcePath) != "" &&
			opts.ExpectedSourceOffsetKnown {
			// Keep the path together with the fingerprint and offset.  The
			// sender must revalidate the same source, and a fingerprint without
			// its identity is an incomplete proof that can never be accepted.
			msg.TranscriptSourcePath = strings.TrimSpace(opts.ExpectedSourcePath)
			msg.TranscriptSourceProofFingerprint = strings.TrimSpace(opts.ExpectedSourceFingerprint)
			msg.TranscriptSourceProofOffset = opts.ExpectedSourceOffset
			msg.TranscriptSourceProofOffsetKnown = true
		}
		if strings.TrimSpace(opts.ExpectedSourceReadFingerprint) != "" &&
			strings.TrimSpace(opts.ExpectedSourcePath) != "" &&
			opts.ExpectedSourceReadRangeKnown {
			msg.TranscriptSourcePath = strings.TrimSpace(opts.ExpectedSourcePath)
			msg.TranscriptSourceReadProofFingerprint = strings.TrimSpace(opts.ExpectedSourceReadFingerprint)
			msg.TranscriptSourceReadProofStartOffset = opts.ExpectedSourceReadStartOffset
			msg.TranscriptSourceReadProofEndOffset = opts.ExpectedSourceReadEndOffset
			msg.TranscriptSourceReadProofRangeKnown = true
		}
		if isFinal {
			// Persist one group identity on every terminal chunk.  Failure fencing
			// must cover continuation parts too; NotificationKind is intentionally
			// only set on the owner/notification part for existing side-effect
			// semantics.
			msg.TerminalGroupID = "terminal-final:" + strings.TrimSpace(turnID)
		}
		if trustedMath {
...
func validateFinalOutboxChunk(intended teamstore.OutboxMessage, queued teamstore.OutboxMessage) error {
	if queued.SessionID == intended.SessionID &&
		...
		(strings.TrimSpace(queued.TerminalGroupID) == "" || strings.TrimSpace(intended.TerminalGroupID) == "" || strings.TrimSpace(queued.TerminalGroupID) == strings.TrimSpace(intended.TerminalGroupID)) &&
...
func (b *Bridge) queueOrSendTranscriptDeliveryChunksWithOptions(ctx context.Context, session Session, local codexhistory.Session, record TranscriptRecord, checkpointLine int, checkpointOffset int64, kind string, text string, opts outboxQueueOptions, turnID string, checkpointID string, advanceCheckpoint bool, queueOnly bool, deliveryNamespace string) error {
	...
	if transcriptOutboxAutomaticRun(turnID) && strings.TrimSpace(opts.ExpectedSourceFingerprint) == "" {
		...
		// to quarantine after the checkpoint writer has rejected it. Keeping the
		// queue and cursor decision together prevents a legacy blocked checkpoint
	...
	for i, chunk := range chunks {
		...
		queuedMsg, _, _, err := b.store.QueueTranscriptDeliveryOutbox(ctx, teamstore.TranscriptDeliveryQueueRequest{
			...
			Checkpoint: teamstore.ImportCheckpoint{
				...
				SessionID:         session.ID,
				SourcePath:        opts.ExpectedSourcePath,
...
func (b *Bridge) queueAndSendOutbox(ctx context.Context, msg teamstore.OutboxMessage) error {
	...
	if retried, err := b.retryKnownAmbiguousOutbox(ctx, queued); retried {
		return err
	}
	return b.flushPendingOutboxFilteredWithOptions(ctx, "", "", queued.TeamsChatID, outboxFlushOptions{
		AllowProtectedAmbiguousBypass: outboxTurnAllowsProtectedAmbiguousBypass(queued.TurnID),
		SkipUnresolvedTranscript:      true,
	})
}
...
func (b *Bridge) queueAndBestEffortSendOutbox(ctx context.Context, msg teamstore.OutboxMessage) error {
	...
	if retried, retryErr := b.retryKnownAmbiguousOutbox(ctx, queued); retried {
		if retryErr != nil && b.out != nil {
			_, _ = fmt.Fprintf(b.out, "Teams best-effort outbox retry error: %v\n", retryErr)
		}
		return nil
	}
	if err := b.flushPendingOutboxFilteredWithOptions(ctx, "", "", queued.TeamsChatID, outboxFlushOptions{
		AllowProtectedAmbiguousBypass: outboxTurnAllowsProtectedAmbiguousBypass(queued.TurnID),
		SkipUnresolvedTranscript:      true,
	}); err != nil && b.out != nil {
		_, _ = fmt.Fprintf(b.out, "Teams best-effort outbox send error: %v\n", err)
...
func (b *Bridge) flushPendingOutboxFiltered(ctx context.Context, sessionID string, turnID string, chatID string) error {
	return b.flushPendingOutboxFilteredWithOptions(ctx, sessionID, turnID, chatID, outboxFlushOptions{SkipUnresolvedTranscript: true})
}
...
type outboxFlushOptions struct {
	...
	// unknown.  It never retries or retires the older row; it only prevents a
	// lost response from permanently deadlocking the chat.  Automatic history
	// rows and another chunk of the same turn remain ordered behind it.
...
func transcriptOutboxBlockedByUnresolvedAnchor(ctx context.Context, store *teamstore.Store, msg teamstore.OutboxMessage, anchorCache map[string]teamstore.ExecutionAnchor, anchorKnown map[string]bool) bool {
	...
	if store == nil || sessionID == "" || !isTranscriptAnswerOutbox(msg) {
		return false
	}
	if anchorCache == nil {
	...
	if !anchorKnown[sessionID] {
		state, err := store.SessionExecutionStateSnapshot(ctx, sessionID, transcriptCheckpointID(sessionID))
		if err != nil {
			// A state-read failure must not turn the global outbox flusher into a
			// fail-open path. Defer transcript answers until ownership can be
			// inspected again.
			anchorCache[sessionID] = teamstore.ExecutionAnchor{SessionID: sessionID, State: executionAnchorStateUnresolved}
			anchorKnown[sessionID] = true
			return true
		}
		anchorKnown[sessionID] = true
		if checkpoint, ok := state.ImportCheckpoints[transcriptCheckpointID(sessionID)]; ok && executionAnchorActive(checkpoint.UnresolvedExecution) {
			anchorCache[sessionID] = *checkpoint.UnresolvedExecution
		} else if _, ambiguous := unresolvedAmbiguousCodexTurn(state, Session{ID: sessionID}); ambiguous {
			// Legacy interrupted turns predate the JSON anchor. Treat them as
			// unresolved at the outbox boundary too; the next sync pass will
			// materialize the anchor, but a concurrent flusher must not release
			// old transcript answers before that migration happens.
			anchorCache[sessionID] = teamstore.ExecutionAnchor{SessionID: sessionID, State: executionAnchorStateUnresolved}
		}
	}
	anchor, ok := anchorCache[sessionID]
	if !ok || !executionAnchorActive(&anchor) {
		return false
	}
	turnID := strings.TrimSpace(msg.TurnID)
	// Explicit operator-directed history publication is the one deliberate
	// bypass. It remains labeled history and never authorizes a new Codex turn.
	if strings.HasPrefix(turnID, "publish-history:") || strings.HasPrefix(turnID, "publish-full:") {
		return false
	}
...
// transcriptOutboxTrustedBeforeAnchor is the bridge-side counterpart of the
// store's final CAS rule. Only a row carrying an exact parent checkpoint,
// source path, and source offset at or before the trusted cutoff can bypass an
// active anchor. Legacy rows without these fields remain blocked.
func transcriptOutboxTrustedBeforeAnchor(msg teamstore.OutboxMessage, anchor teamstore.ExecutionAnchor) bool {
	if strings.TrimSpace(anchor.SourcePath) == "" || anchor.CutoffOffset <= 0 {
		return false
	}
	if strings.TrimSpace(msg.TranscriptCheckpointID) == "" ||
		strings.TrimSpace(msg.TranscriptCheckpointID) != transcriptCheckpointID(anchor.SessionID) ||
		strings.TrimSpace(msg.TranscriptSourcePath) == "" ||
		cleanComparablePath(msg.TranscriptSourcePath) != cleanComparablePath(anchor.SourcePath) ||
		!msg.TranscriptSourceOffsetKnown {
		return false
	}
...
// captured when an automatic linked-transcript row was queued.  It is a
// sender-side fence, not a replacement for the store checkpoint CAS: a source
// may still change after this check, but a replacement/rewrite between scan,
// queue, and Graph POST cannot silently turn the queued stale record into a
// published answer. Rows without a proof are legacy or explicit-history rows
// and retain their existing conservative ownership checks.
func transcriptOutboxSourceProofMatches(msg teamstore.OutboxMessage) bool {
	expected := strings.TrimSpace(msg.TranscriptSourceProofFingerprint)
	if expected == "" {
		// Ordinary user/control outbox rows do not carry transcript provenance.
		// Keep this predicate allocation-free and avoid the more detailed kind
		// classification on the hot send path; automatic sync/import namespaces
		// and rows with any source metadata still take the conservative branch.
		turnID := strings.TrimSpace(msg.TurnID)
		if strings.TrimSpace(msg.TranscriptCheckpointID) == "" && strings.TrimSpace(msg.TranscriptSourcePath) == "" &&
			!msg.TranscriptSourceOffsetKnown && strings.TrimSpace(msg.TranscriptSourceReadProofFingerprint) == "" &&
			!msg.TranscriptSourceReadProofRangeKnown && !strings.HasPrefix(turnID, "sync:") &&
			!strings.HasPrefix(turnID, "import:") && !strings.HasPrefix(turnID, "import-bg:") {
			return true
		}
		// A linked-transcript row written by an older helper may have no bounded
		// proof at all.  Once it carries transcript provenance (or has one of the
		// automatic sync/import kinds), treating the missing proof as a match would
		// let a source replacement turn a queued legacy row into a stale answer.
		// Explicit history publication is the only deliberate exception: it is a
		// user-directed reread which establishes a new boundary.
		return !transcriptOutboxRequiresSourceProof(msg)
	}
	path := strings.TrimSpace(msg.TranscriptSourcePath)
	if path == "" || !msg.TranscriptSourceProofOffsetKnown || msg.TranscriptSourceProofOffset < 0 {
		return false
	}
	pathInfo, err := os.Stat(path)
	if err != nil || pathInfo.IsDir() || pathInfo.Size() < msg.TranscriptSourceProofOffset {
		return false
	}
	f, err := os.Open(path)
	if err != nil {
	...
	defer f.Close()
	fdInfo, err := f.Stat()
	if err != nil || fdInfo.IsDir() || fdInfo.Size() < msg.TranscriptSourceProofOffset || !os.SameFile(pathInfo, fdInfo) {
		return false
	}
	actual := strings.TrimSpace(transcriptCheckpointSourceFingerprintFromReader(f, path, fdInfo.Size(), msg.TranscriptSourceProofOffset))
	if actual == "" || actual != expected {
		return false
	}
	postInfo, err := os.Stat(path)
	if err != nil || postInfo.IsDir() || !os.SameFile(pathInfo, postInfo) || postInfo.Size() < msg.TranscriptSourceProofOffset {
		return false
	}
	if strings.TrimSpace(msg.TranscriptSourceReadProofFingerprint) != "" || msg.TranscriptSourceReadProofRangeKnown {
		if !msg.TranscriptSourceReadProofRangeKnown || msg.TranscriptSourceReadProofStartOffset < 0 ||
			...
			postInfo.Size() < msg.TranscriptSourceReadProofEndOffset {
			return false
		}
		readProof := outboxQueueOptions{
			ExpectedSourcePath:            path,
			ExpectedSourceReadFingerprint: msg.TranscriptSourceReadProofFingerprint,
			ExpectedSourceReadStartOffset: msg.TranscriptSourceReadProofStartOffset,
			ExpectedSourceReadEndOffset:   msg.TranscriptSourceReadProofEndOffset,
			ExpectedSourceReadRangeKnown:  msg.TranscriptSourceReadProofRangeKnown,
		}
		if !transcriptSourceReadProofMatches(readProof) {
			return false
		}
	}
	return true
...
type transcriptSourceProofCacheEntry struct {
...
// source read during the pre-claim checks of one flush. The cache is only a
// cheap stat-validated hint: every pre-POST and post-POST boundary still
// forces a fresh content proof, and the store CAS remains authoritative.
func transcriptOutboxSourceProofMatchesWithCache(msg teamstore.OutboxMessage, cache map[string]transcriptSourceProofCacheEntry, force bool) bool {
	if !force && cache != nil {
		key := transcriptSourceProofCacheKey(msg)
		if entry, ok := cache[key]; ok && transcriptSourceProofCacheEntryMatches(entry, msg) {
			return true
		}
	}
	ok := transcriptOutboxSourceProofMatches(msg)
	if ok && cache != nil && strings.TrimSpace(msg.TranscriptSourceProofFingerprint) != "" {
		if len(cache) < transcriptSourceProofCacheMaxEntries {
			if info, err := os.Stat(strings.TrimSpace(msg.TranscriptSourcePath)); err == nil && !info.IsDir() {
				cache[transcriptSourceProofCacheKey(msg)] = transcriptSourceProofCacheEntry{
					ExpectedFingerprint: strings.TrimSpace(msg.TranscriptSourceProofFingerprint),
					Path:                strings.TrimSpace(msg.TranscriptSourcePath),
					Offset:              msg.TranscriptSourceProofOffset,
					OffsetKnown:         msg.TranscriptSourceProofOffsetKnown,
					ReadFingerprint:     strings.TrimSpace(msg.TranscriptSourceReadProofFingerprint),
					ReadStart:           msg.TranscriptSourceReadProofStartOffset,
...
func transcriptSourceProofCacheKey(msg teamstore.OutboxMessage) string {
	return strings.Join([]string{
		strings.TrimSpace(msg.TranscriptSourcePath),
		strings.TrimSpace(msg.TranscriptSourceProofFingerprint),
		strconv.FormatInt(msg.TranscriptSourceProofOffset, 10),
		strconv.FormatBool(msg.TranscriptSourceProofOffsetKnown),
		strings.TrimSpace(msg.TranscriptSourceReadProofFingerprint),
		strconv.FormatInt(msg.TranscriptSourceReadProofStartOffset, 10),
		strconv.FormatInt(msg.TranscriptSourceReadProofEndOffset, 10),
		strconv.FormatBool(msg.TranscriptSourceReadProofRangeKnown),
	}, "\x00")
}

func transcriptSourceProofCacheEntryMatches(entry transcriptSourceProofCacheEntry, msg teamstore.OutboxMessage) bool {
	if entry.Info == nil || entry.ExpectedFingerprint != strings.TrimSpace(msg.TranscriptSourceProofFingerprint) ||
		entry.Path != strings.TrimSpace(msg.TranscriptSourcePath) || entry.Offset != msg.TranscriptSourceProofOffset ||
		entry.OffsetKnown != msg.TranscriptSourceProofOffsetKnown || entry.ReadFingerprint != strings.TrimSpace(msg.TranscriptSourceReadProofFingerprint) ||
		entry.ReadStart != msg.TranscriptSourceReadProofStartOffset || entry.ReadEnd != msg.TranscriptSourceReadProofEndOffset ||
		entry.ReadRangeKnown != msg.TranscriptSourceReadProofRangeKnown {
		return false
	}
...
func transcriptOutboxRequiresSourceProof(msg teamstore.OutboxMessage) bool {
	if !isTranscriptAnswerOutbox(msg) {
		return false
	}
	turnID := strings.TrimSpace(msg.TurnID)
	// Explicit history publication is an operator-selected reread and may
	// establish a new boundary for a legacy transcript. Both `import:` and
	// `import-bg:` are automatic linked-history runs; they must carry a source
	// proof before reaching Graph. Exempting either one would let a missing
	// proof fail open in the sender after the row had already been queued.
	...
	// Source-less legacy rows are handled by the state-aware
	// transcriptOutboxLegacySourceRewriteBlocked fence before this predicate is
	// used. Returning false here means only that the row has no own proof to
	// validate; it must not bypass that fence or establish a safe boundary.
	return false
}

// transcriptAutomaticImportSourceProofMatches verifies the source prefix
// captured before an automatic import began.  The fingerprint includes the
// source identity and the bounded content window, so this catches an atomic
// replacement or an in-place rewrite between read, queue, and checkpoint
// without reading the complete transcript.  It is used only on the cold
// import path; ordinary poll/admission paths do not call it.
func transcriptAutomaticImportSourceProofMatches(proof outboxQueueOptions) bool {
	if strings.TrimSpace(proof.ExpectedSourcePath) == "" ||
		strings.TrimSpace(proof.ExpectedSourceFingerprint) == "" ||
		!proof.ExpectedSourceOffsetKnown || proof.ExpectedSourceOffset < 0 {
		return false
	}
	if transcriptCheckpointSourceFingerprint(proof.ExpectedSourcePath, proof.ExpectedSourceOffset) != strings.TrimSpace(proof.ExpectedSourceFingerprint) {
		return false
	}
	return transcriptSourceReadProofMatches(proof)
}

func transcriptSourceProofQueueOptionsMatches(proof outboxQueueOptions) bool {
	if strings.TrimSpace(proof.ExpectedSourcePath) == "" ||
		strings.TrimSpace(proof.ExpectedSourceFingerprint) == "" ||
		!proof.ExpectedSourceOffsetKnown || proof.ExpectedSourceOffset < 0 {
		return false
...
func transcriptSourceReadProofMatches(proof outboxQueueOptions) bool {
	if strings.TrimSpace(proof.ExpectedSourceReadFingerprint) == "" && !proof.ExpectedSourceReadRangeKnown {
		// Active-turn backfill and legacy callers only carry the cursor proof.
		// Automatic cold imports always populate the stronger read-range proof.
		return true
	}
	if strings.TrimSpace(proof.ExpectedSourceReadFingerprint) == "" || !proof.ExpectedSourceReadRangeKnown ||
		proof.ExpectedSourceReadStartOffset < 0 || proof.ExpectedSourceReadEndOffset < proof.ExpectedSourceReadStartOffset {
		return false
	}
	// A persisted proof is untrusted input.  Keep the final sender-side
...
// transcriptOutboxLegacySourceRewriteBlocked is the state-aware safety fence
// for source-less transcript rows written by older helpers. The row itself
// cannot carry a bounded source proof, so a missing or incomplete canonical
// boundary is ambiguity and must block automatic delivery. Explicit history
// rows are exempt above and establish their own operator-selected boundary.
func (b *Bridge) transcriptOutboxLegacySourceRewriteBlocked(ctx context.Context, msg teamstore.OutboxMessage) bool {
	if b == nil || b.store == nil || transcriptOutboxUserExplicitHistory(msg.TurnID) {
		return false
	}
	if !teamstore.IsAutomaticTranscriptOutbox(msg) {
		return false
	}
	if strings.TrimSpace(msg.SessionID) == "" {
		// An automatic transcript row without a session cannot be associated
		// with a canonical checkpoint, so it is never safe to send.
		return true
	}
	if strings.TrimSpace(msg.TranscriptSourceProofFingerprint) != "" {
		// Complete source proofs are checked by transcriptOutboxSourceProofMatches
		// immediately before delivery. A fingerprint without its path/offset is
		// malformed state and must not bypass the legacy fence.
		return strings.TrimSpace(msg.TranscriptSourcePath) == "" || !msg.TranscriptSourceProofOffsetKnown || msg.TranscriptSourceProofOffset < 0
	}
	checkpoint, found, err := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(msg.SessionID))
	if err != nil || !found {
		return true
	}
	...
	// A legacy outbox row has no source proof of its own.  If the canonical
	// checkpoint does have one, verify it at send time as a compatibility fence
	// too; this catches same-size/same-mtime rewrites even when the periodic
	// HistoryWatch stat poll has not reached its reconcile pass yet.
	if strings.TrimSpace(checkpoint.SourcePath) != "" && strings.TrimSpace(checkpoint.SourceFingerprint) != "" && linkedCheckpointOffsetKnown(checkpoint) {
		return !linkedCheckpointPrefixMatches(checkpoint.SourcePath, checkpoint)
	}
	// A legacy row without a bounded source proof cannot be safely associated
	// with the current transcript after a rewrite.  Keep it queued until an
	// explicit history operation supplies the boundary.
	return true
}

func (b *Bridge) fenceTranscriptOutboxSourceRewrite(ctx context.Context, msg teamstore.OutboxMessage) error {
	if b == nil || b.store == nil {
		return nil
	}
	if !transcriptOutboxUserExplicitHistory(msg.TurnID) && strings.TrimSpace(msg.SessionID) != "" {
		checkpointID := firstNonEmptyString(msg.TranscriptCheckpointID, transcriptCheckpointID(msg.SessionID))
		checkpoint, found, err := b.store.ImportCheckpoint(ctx, checkpointID)
		if err != nil {
			return err
		}
		if !found {
			checkpoint = teamstore.ImportCheckpoint{ID: checkpointID, SessionID: msg.SessionID, SourcePath: msg.TranscriptSourcePath}
		}
		sourcePath := firstNonEmptyString(msg.TranscriptSourcePath, checkpoint.SourcePath)
		session := b.reg.SessionByID(msg.SessionID)
		if session == nil {
			session = &Session{ID: msg.SessionID, CodexThreadID: msg.CodexThreadID}
		}
		if err := b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, *session, sourcePath, checkpoint); err != nil {
			return err
		}
	}
	return nil
...
// retireTranscriptOutboxBeforePostSourceRewrite handles a source-proof failure
// after the outbox lease was claimed but before any Graph POST. The row must
// not be retried as the same transcript, and it must not be marked skipped
// without first fencing the canonical checkpoint: otherwise a checkpoint that
// won the queue race could make the rewritten tail look already delivered.
func (b *Bridge) retireTranscriptOutboxBeforePostSourceRewrite(ctx context.Context, msg teamstore.OutboxMessage, reason string) error {
	if err := b.fenceTranscriptOutboxSourceRewrite(ctx, msg); err != nil {
		return err
	}
	_, err := b.store.MarkOutboxSkippedForAttempt(ctx, msg.ID, msg.SendAttemptToken, reason)
	return err
}

func transcriptOutboxUserExplicitHistory(turnID string) bool {
	turnID = strings.TrimSpace(turnID)
	return strings.HasPrefix(turnID, "publish-history:") || strings.HasPrefix(turnID, "publish-full:")
}

// outboxTurnAllowsAmbiguousRetry is deliberately narrower than the general
// transcript-history classifier.  A normal outbox flush must never release a
// Sending row whose Graph outcome is unknown. Only an operator-selected
// history publication may choose that recovery boundary; background imports
// use the import-bg namespace and remain fail-closed.
func outboxTurnAllowsAmbiguousRetry(turnID string) bool {
	return transcriptOutboxUserExplicitHistory(turnID)
}

func outboxTurnAllowsProtectedAmbiguousBypass(turnID string) bool {
	turnID = strings.TrimSpace(turnID)
...
// outboxDeliverySupersedable covers low-value control/progress rows whose
// ambiguous POST must not hold a later, distinct response behind FIFO forever.
// It is intentionally separate from Store's turn-lifecycle transient class:
// the same deterministic ACK/helper row must remain retryable when the
// originating command is retried in-process.  This predicate is used only
// after a different later outbox row is already waiting.
func outboxDeliverySupersedable(msg teamstore.OutboxMessage) bool {
	if teamstore.OutboxDeliveryProtected(msg) {
		return false
	}
...
// outboxDeliverySupersedable.  A protected final/attachment must never be
// retried after an ambiguous POST, but it also must not make a Work chat
// unusable forever when a later user turn arrives.  The old row remains in
// Sending with its unknown external outcome; this predicate only permits the
// later distinct delivery to proceed.
func outboxCanBypassProtectedAmbiguousPredecessor(current teamstore.OutboxMessage, earlier teamstore.OutboxMessage) bool {
	if !teamstore.OutboxSendIsAmbiguous(earlier) || !teamstore.OutboxDeliveryProtected(earlier) {
		return false
	}
	...
	earlierTurnID := strings.TrimSpace(earlier.TurnID)
	if currentTurnID != "" {
		if currentTurnID == earlierTurnID || transcriptOutboxUserExplicitHistory(currentTurnID) {
			return false
		}
		for _, prefix := range []string{"sync:", "import:", "import-bg:", "publish-history:", "publish-full:"} {
			if strings.HasPrefix(currentTurnID, prefix) {
				return false
			}
		}
		// A normal turn ID is sufficient evidence that this is a distinct
		// user request.  The durable Turn queue already serializes execution;
		// this only changes the outbound Graph FIFO fence.
		return true
	}
	// Direct helper/control output has no TurnID.  It is safe to let a status,
	// error, or acknowledgement explain/recover the chat while the unknown
	// protected delivery remains quarantined.
	kind := strings.ToLower(strings.TrimSpace(current.Kind))
	return kind == "ack" || kind == "helper" || strings.HasPrefix(kind, "helper-") || strings.HasPrefix(kind, "error")
}

// obsoleteHistoryOutboxPredecessor identifies only automatic history-policy
// rows that an explicit operator-selected history import is allowed to pass.
// Protected answers and other durable user-visible work remain ordered; this
// exception exists solely so a stale gate notice or a fenced accepted row
// cannot deadlock the recovery command behind the policy it is repairing.
func obsoleteHistoryOutboxPredecessor(msg teamstore.OutboxMessage) bool {
	if teamstore.OutboxDeliveryProtected(msg) {
		return false
	}
	if teamstore.IsLegacyHistoryGateNotice(msg) || teamstore.IsLegacyUnverifiableTranscriptOutbox(msg) {
		return true
	}
	return msg.BlockedBySourceRewrite && msg.Status == teamstore.OutboxStatusAccepted && strings.TrimSpace(msg.TeamsMessageID) != ""
}

// earlierUnsentOutboxSkippingLegacy retires only stale pre-source-proof
// transcript rows while walking FIFO. Queued rows have never acquired a send
// lease; Sending rows are eligible only after their lease expires and bounded
// reconciliation has failed. Fresh rows, accepted identities, and rows with
// source metadata remain fail-closed and are returned.
func (b *Bridge) earlierUnsentOutboxSkippingLegacy(ctx context.Context, msg teamstore.OutboxMessage) (teamstore.OutboxMessage, bool, error) {
	if b == nil || b.store == nil {
		return teamstore.OutboxMessage{}, false, nil
	}
	for {
		earlier, ok, err := b.store.EarlierUnsentOutbox(ctx, msg)
		if err != nil || !ok {
			return earlier, ok, err
		}
...
func isTranscriptAnswerOutbox(msg teamstore.OutboxMessage) bool {
	kind := strings.ToLower(strings.TrimSpace(msg.Kind))
	if strings.EqualFold(strings.TrimSpace(msg.NotificationKind), "needs_attention") {
		// Recovery/control notices must remain deliverable even when an older
		// transcript answer is quarantined behind an unresolved execution.
		return false
	}
	if strings.HasSuffix(kind, "-needs-attention") || kind == "needs-attention" {
		return false
	}
	if strings.HasPrefix(kind, "sync-status-") || strings.HasPrefix(kind, "sync-complete") || strings.HasPrefix(kind, "import-title") || strings.HasPrefix(kind, "import-complete") {
		return false
	}
	return isTranscriptImportBatchOutboxKind(kind) ||
		strings.HasPrefix(kind, "import-") || strings.HasPrefix(kind, "import-bg-") ||
		strings.HasPrefix(kind, "sync-") || strings.HasPrefix(kind, "publish-full-") ||
		...
		isFinalOutboxKind(kind) || strings.HasPrefix(kind, "answer") || strings.HasPrefix(kind, "final-answer") ||
...
func (b *Bridge) flushPendingOutboxFilteredWithOptions(ctx context.Context, sessionID string, turnID string, chatID string, opts outboxFlushOptions) error {
	...
	err := func() error {
		...
		var anchorKnown map[string]bool
		var sourceProofCache map[string]transcriptSourceProofCacheEntry
		recoveryProbeBudget := 1
		recoveryPageBudget := outboxRecoveryMaxPagesPerFlush
		sent := 0
		sentBytes := 0
		...
		for {
			...
			for _, msg := range pending {
				...
				sent++
				sentBytes += len(msg.Body)
				if opts.MaxMessages > 0 && sent >= opts.MaxMessages {
					budgetExhausted = true
					break
				}
				if opts.MaxBytes > 0 && sentBytes >= opts.MaxBytes {
					budgetExhausted = true
					break
				}
			}
...
type outboxSendOptions struct {
	...
	// outbox ID. RecoveryPageBudget applies the same bound to Graph continuation
	// pages; direct single-row sends leave both fields nil.
	RecoveryProbeBudget *int
	RecoveryPageBudget  *int
	SentSideEffects     *[]sentOutboxSideEffect
	SourceProofCache    map[string]transcriptSourceProofCacheEntry
}
...
func (b *Bridge) sendQueuedOutboxWithOptions(ctx context.Context, outbox teamstore.OutboxMessage, opts outboxSendOptions) error {
	...
	// normally retires queued and lease-expired rows after the owner lease is
	// acquired, but keep the sender fail-closed as a second boundary for rows
	// created by another process or missed by the migration query. A fresh
	// Sending row is left alone until its external POST is recoverable or its
	// lease expires; skipping it here would hide an ambiguous Graph side effect.
	if (outbox.Status == teamstore.OutboxStatusQueued || outbox.Status == teamstore.OutboxStatusSending) && !teamstore.OutboxDeliveryProtected(outbox) && teamstore.IsLegacyHistoryGateNoticeForSend(outbox) {
		if outbox.Status == teamstore.OutboxStatusSending && !teamstore.OutboxSendLeaseExpired(outbox, time.Now()) {
		...
		if outbox.Status == teamstore.OutboxStatusSending {
			// A stale snapshot must not retire a new attempt claimed after this
			// sender read the row.  Startup migration may use the unscoped method
			// only while the sender is quiesced; the live sender needs the attempt
			// CAS even for an obsolete diagnostic.
			_, err = b.store.MarkOutboxSkippedForAttempt(ctx, outbox.ID, outbox.SendAttemptToken, "obsolete automatic history-gate notice")
			if err != nil && strings.TrimSpace(outbox.SendAttemptToken) == "" && errors.Is(err, teamstore.ErrOutboxSendNotClaimed) {
				return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
			}
		} else {
	...
	// A pre-source-proof queued transcript row has no external-send identity and
	// cannot be safely associated with the current local file after an upgrade.
	// It is stale compatibility data, not user work: retire it before the sender
	// can turn it into a Graph POST. The store method repeats the stale-row
	...
	// Source-less transcript rows written by an older helper remain compatible
	// until their checkpoint is explicitly marked source-rewritten.  Perform
	// this state-aware fence after ambiguous-send recovery so an in-flight Graph
	// request still has a chance to reconcile its stable message ID.
	if outbox.BlockedBySourceRewrite || b.transcriptOutboxLegacySourceRewriteBlocked(ctx, outbox) {
		...
		if outbox.Status == teamstore.OutboxStatusSending && strings.TrimSpace(outbox.TeamsMessageID) == "" && strings.TrimSpace(outbox.SendAttemptToken) != "" {
			_, markErr := b.store.MarkOutboxAmbiguousSendErrorForAttempt(ctx, outbox.ID, outbox.SendAttemptToken, "transcript source provenance changed after an ambiguous Graph send")
		...
		if outbox.Status == teamstore.OutboxStatusQueued && !outbox.BlockedBySourceRewrite {
			if err := b.fenceTranscriptOutboxSourceRewrite(ctx, outbox); err != nil {
				return err
			}
	...
	// Check the durable source proof before accepting an existing row or
	// claiming a queued row.  A changed source must never be retried as if it
	// were the same transcript; the linked-sync path will block and require an
	// explicit history publication instead.
	if !transcriptOutboxSourceProofMatchesWithCache(outbox, opts.SourceProofCache, false) {
		if outbox.Status == teamstore.OutboxStatusQueued && strings.TrimSpace(outbox.TeamsMessageID) != "" {
			if err := b.fenceTranscriptOutboxSourceRewrite(ctx, outbox); err != nil {
				return err
			}
		...
		if outbox.Status == teamstore.OutboxStatusSending && strings.TrimSpace(outbox.TeamsMessageID) == "" && strings.TrimSpace(outbox.SendAttemptToken) != "" {
			// The Graph read above found no exact provenance marker, but an old
			// Sending row still has an unknown external outcome. Preserve the
			// no-duplicate fence as durable ambiguity. This lets a distinct later
			// user turn bypass a protected stale transcript row while explicit
			// recovery can still reconcile the original marker.
			_, markErr := b.store.MarkOutboxAmbiguousSendErrorForAttempt(ctx, outbox.ID, outbox.SendAttemptToken, "transcript source provenance changed after an ambiguous Graph send")
			if markErr != nil && !errors.Is(markErr, teamstore.ErrOutboxSendNotClaimed) {
				return markErr
			}
		...
		// A queued row has not acquired a Graph lease, so a source-proof
		// mismatch can be retired before claim. Leaving it queued here is a
		// permanent FIFO deadlock: the next pass performs the same proof check,
		// never reaches the fence/skip path, and can never deliver later work.
		// Fence the canonical checkpoint first so a concurrent scanner cannot
		// advance over the rewritten source while this stale row is retired.
		if outbox.Status == teamstore.OutboxStatusQueued && transcriptOutboxHasSourceProof(outbox) {
			if err := b.fenceTranscriptOutboxSourceRewrite(ctx, outbox); err != nil {
				return err
			}
	...
	// A legacy migration can leave a queued row with a durable Graph identity.
	// Promote that identity before claiming the row; otherwise the generic send
	...
	// under the current attempt instead of allowing a stale Sending row to be
	// claimed and POSTed a second time after a restart.
	if outbox.Status == teamstore.OutboxStatusSending && strings.TrimSpace(outbox.TeamsMessageID) != "" {
	...
	if outbox.Status == teamstore.OutboxStatusAccepted && outbox.TeamsMessageID != "" {
		...
		if err == nil {
			if sent.Status != teamstore.OutboxStatusSent || sent.BlockedByUnresolvedExecution || sent.BlockedByTerminalFailure {
				...
				// final CAS.  Keep the row Accepted and let a later flush promote it
				// without another POST; importantly, do not count it as sent or
				// advance a transcript checkpoint in this pass.
				return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
			}
	...
	// Recheck immediately before any retry/recovery/send side effect. The
	// flusher performs an earlier page-level check and passes its per-flush
	// cache here, so ordinary transcript rows do not decode the same session
	// snapshot twice. The post-claim and pre-POST checks below deliberately use
	// fresh caches because an anchor may be persisted during this send.
	anchorCache := opts.AnchorCache
	anchorKnown := opts.AnchorKnown
	if anchorCache == nil {
		anchorCache = make(map[string]teamstore.ExecutionAnchor)
	}
	if anchorKnown == nil {
	...
	if anchorBlocked && outbox.Status == teamstore.OutboxStatusSending && strings.TrimSpace(outbox.SendAttemptToken) != "" {
		...
		// The caller handed us an already-claimed row. Releasing that claim is a
		// durable no-op and must happen before the generic markerless-Sending
		// guard, otherwise the guard would report a misleading deferred error.
		return nil
	}
	if outbox.Status == teamstore.OutboxStatusSending && strings.TrimSpace(outbox.TeamsMessageID) == "" && outbox.BlockedByTerminalFailure {
		// A terminal execution failure fences the final row against a late
		// callback. Its external outcome is still unknown, so do not POST it, but
		// also do not turn the intentionally retained in-flight row into a noisy
		// deferred error on every flush.
		return nil
	}
	// A Sending row without a durable Graph identity has an unknown external
	// outcome, regardless of whether an older version persisted the current
	// ambiguous-error marker. Never release it back to Queued: doing so can
	// replay a POST that Graph already accepted. A row with an attempt token is
	// upgraded to the current durable ambiguous form; pre-CAS rows remain
	// deferred for explicit reconciliation because there is no safe CAS owner.
	if outbox.Status == teamstore.OutboxStatusSending && strings.TrimSpace(outbox.TeamsMessageID) == "" {
		if !teamstore.OutboxSendIsAmbiguous(outbox) && strings.TrimSpace(outbox.SendAttemptToken) != "" {
			if _, err := b.store.MarkOutboxAmbiguousSendErrorForAttempt(ctx, outbox.ID, outbox.SendAttemptToken, "legacy Sending row has no durable Graph identity"); err != nil && !errors.Is(err, teamstore.ErrOutboxSendNotClaimed) {
				return err
			}
		}
		// A normal flush must never replay a POST whose external outcome is
		// unknown. Explicit history/publish recovery may reconcile an exact
		// durable marker, but a complete no-match scan is not proof that the POST
		// was rejected; releasing it would allow a duplicate POST. Pre-CAS rows
		// without a token can never be reconciled by this sender.
		...
		if opts.AllowAmbiguousRetry && opts.RecoveryProbeBudget != nil && *opts.RecoveryProbeBudget <= 0 {
			return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
		}
	}
	if opts.AllowAmbiguousRetry && teamstore.OutboxSendIsAmbiguous(outbox) && !teamstore.OutboxDeliveryProtected(outbox) && !isForkDeliveryOutbox(outbox) {
		...
		// Non-transient ambiguity is intentionally retained. The Graph read
		// path can settle an exact marker, but it must not turn an empty window
		// into permission to replay the POST.
		return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
	}
	...
	if !opts.IgnoreEarlierOutbox {
		currentIsExplicitHistory := transcriptOutboxUserExplicitHistory(outbox.TurnID)
		var earlier teamstore.OutboxMessage
		var ok bool
		var err error
		if currentIsExplicitHistory {
			// An explicit recovery may pass several obsolete predecessors, but it
			// must stop at the first protected row. Looking at only the earliest
			// predecessor would incorrectly jump over a protected answer that
			// followed an obsolete notice.
			candidates, candidateErr := b.store.EarlierUnsentOutboxes(ctx, outbox)
			if candidateErr != nil {
				return candidateErr
			}
			for _, candidate := range candidates {
				if obsoleteHistoryOutboxPredecessor(candidate) {
					continue
				}
				earlier, ok = candidate, true
				break
			}
		} else {
			earlier, ok, err = b.earlierUnsentOutboxSkippingLegacy(ctx, outbox)
		}
		if err != nil {
			return err
		} else if ok {
			if opts.SkipUnresolvedTranscript && transcriptOutboxBlockedByUnresolvedAnchor(ctx, b.store, earlier, map[string]teamstore.ExecutionAnchor{}, map[string]bool{}) {
				// A quarantined transcript answer must not prevent an actionable
				// helper/needs_attention notice from reaching the user. Preserve
				// ordering for another transcript answer, while safe control and
			...
			} else if teamstore.OutboxSendIsAmbiguous(earlier) &&
				teamstore.OutboxDeliveryTransient(earlier) &&
				!teamstore.OutboxDeliveryProtected(earlier) &&
				strings.TrimSpace(earlier.SendAttemptToken) != "" {
				// A progress/queued-status row has no terminal user-visible
				// meaning. If its POST outcome is ambiguous, dropping that one
				// progress update is safer than allowing it to hold a later final
				// answer behind an unresolvable FIFO predecessor. This does not
				// release or replay the POST; it only marks the non-critical row
				// skipped under its attempt CAS.
				if _, err := b.store.MarkOutboxSkippedForAttempt(ctx, earlier.ID, earlier.SendAttemptToken, "ambiguous transient progress superseded by later outbox"); err != nil && !errors.Is(err, teamstore.ErrOutboxSendNotClaimed) {
					return err
				}
				b.forgetOutboxEchoAttempt(earlier.ID)
			} else if teamstore.OutboxSendIsAmbiguous(earlier) &&
				outboxDeliverySupersedable(earlier) &&
				strings.TrimSpace(earlier.SendAttemptToken) != "" &&
				strings.TrimSpace(earlier.ID) != strings.TrimSpace(outbox.ID) {
				// ACK/helper output is retryable when its own deterministic command
				// is replayed, but an unknown POST from an older distinct row must
				// not hold a later user-visible response forever.  Retiring this
				// row does not replay or claim that Graph rejected it; it only
				// releases FIFO for the later row under the attempt CAS.
				if _, err := b.store.MarkOutboxSkippedForAttempt(ctx, earlier.ID, earlier.SendAttemptToken, "ambiguous control output superseded by later outbox"); err != nil && !errors.Is(err, teamstore.ErrOutboxSendNotClaimed) {
					return err
				}
				b.forgetOutboxEchoAttempt(earlier.ID)
	...
	// The source-rewrite marker may be committed after the store claim.  Re-read
	// it before doing any attachment or Graph work; returning the lease to Queued
	// is safe here because no external request has been made yet.  The store-side
	// claim check above closes the already-marked case, while this second check
	// closes the claim/marker race without turning Sending into Skipped.
	if b.transcriptOutboxLegacySourceRewriteBlocked(ctx, outbox) {
		_, markErr := b.store.MarkOutboxSendErrorForAttempt(ctx, outbox.ID, outbox.SendAttemptToken, "transcript source rewrite became blocked after outbox claim")
		if markErr != nil {
			return markErr
		}
		return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
	...
	if transcriptOutboxBlockedByUnresolvedAnchor(ctx, b.store, outbox, map[string]teamstore.ExecutionAnchor{}, map[string]bool{}) {
		_, markErr := b.store.MarkOutboxSendErrorForAttempt(context.Background(), outbox.ID, outbox.SendAttemptToken, "execution ownership became unresolved after outbox claim")
		if markErr != nil {
			return markErr
		}
		return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
	...
	if transcriptOutboxBlockedByUnresolvedAnchor(ctx, b.store, outbox, nil, nil) {
		_, markErr := b.store.MarkOutboxSendErrorForAttempt(context.Background(), outbox.ID, outbox.SendAttemptToken, "execution ownership became unresolved before Graph POST")
		if markErr != nil {
			return markErr
		}
		return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
	}
	// Attachments can yield to another goroutine long enough for the source
	// rewrite marker to appear.  Check the durable marker again immediately
	// before Graph POST; an in-flight marker must never release this row into an
	// automatic retry, and it must not be converted to Skipped while still
	// pre-send.
	if b.transcriptOutboxLegacySourceRewriteBlocked(ctx, outbox) {
		markErr := b.retireTranscriptOutboxBeforePostSourceRewrite(context.Background(), outbox, "transcript source provenance changed before Graph POST")
		if markErr != nil {
			return markErr
		}
		return nil
	...
	if !transcriptOutboxSourceProofMatchesWithCache(outbox, opts.SourceProofCache, true) {
		markErr := b.retireTranscriptOutboxBeforePostSourceRewrite(ctx, outbox, "transcript source provenance changed before Graph POST")
		if markErr != nil {
			return markErr
		}
		return nil
	...
	// Keep the in-memory echo/registry identity before the durable Accepted CAS:
	// if that CAS fails (for example during a store repair), the same Bridge can
	...
	// before registry/global-ledger publication and before the final Sent CAS,
	...
	// is reconciled by the existing marker probe; the extra Accepted transaction
	...
	if err == nil {
		...
		if sent.Status != teamstore.OutboxStatusSent || sent.BlockedByUnresolvedExecution || sent.BlockedByTerminalFailure {
			// Graph has already accepted the message and the durable outbox keeps
			// its ID.  Defer only the side effects/checkpoint; never retry POST.
			b.forgetOutboxEchoAttempt(outbox.ID)
			return outboxDeliveryDeferredError{ChatID: outbox.TeamsChatID, Until: time.Now().Add(2 * time.Minute)}
		}
		b.forgetOutboxEchoAttempt(outbox.ID)
...
func (b *Bridge) recordSentOutboxSideEffectWithOptions(ctx context.Context, outbox teamstore.OutboxMessage, msg ChatMessage, opts outboxSendOptions, sideOpts sentOutboxSideEffectOptions) {
	...
	if outbox.BlockedByUnresolvedExecution || outbox.BlockedByTerminalFailure || outbox.BlockedBySourceRewrite {
		...
		// turn-completion side effects until the execution anchor is resolved.
		return
	}
...
func (e outboxDeliveryDeferredError) Error() string {
	// This error also represents safety fences (source proof, unresolved
	// execution ownership, and ambiguous Graph outcomes). Calling all of those
	// cases a rate limit makes normal recovery look like a transport problem and
	// encourages pointless retries. Actual Graph 429s retain their typed status
	// error and are handled by isGraphRateLimitError.
	return fmt.Sprintf("Teams delivery is deferred until %s", e.Until.Format(time.RFC3339Nano))
}
...
func (b *Bridge) publishCodexSessionLocalWithOptions(ctx context.Context, local codexhistory.Session, project codexhistory.Project, opts publishCodexSessionOptions) (string, error) {
	if existing := b.reg.SessionByCodexThreadID(local.SessionID); existing != nil && isActiveSessionStatus(existing.Status) {
		...
		if checkpoint, found, checkpointErr := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(existing.ID)); checkpointErr != nil {
			return "", checkpointErr
		} else if found && checkpoint.Status == importCheckpointStatusBlocked {
		...
		if historySyncNeedsAttention {
			...
			// Automatic imports may return nil after deliberately fencing a
			// legacy/source-proof-less checkpoint.  That is the intended silent
			// background policy, but this caller still needs to report a truthful
			// result when it was invoked by an explicit publish/continue command;
			// otherwise it claims that history was imported although no title or
			// transcript row was queued.
			checkpoint, found, checkpointErr := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(existing.ID))
			if checkpointErr != nil {
				return "", checkpointErr
			}
...
type transcriptImportResult struct {
	...
	SourceProof       outboxQueueOptions
	UnsafeDiagnostics bool
...
func (b *Bridge) importCodexTranscriptToTeamsWithTarget(ctx context.Context, session Session, local codexhistory.Session, checkpointID string, importTurnID string, kindPrefix string, opts transcriptImportRunOptions) error {
	...
	// A normal publish/continue is an automatic history refresh, not an
	// operator-selected recovery boundary. If an unresolved execution anchor is
	// already durable, keep this path completely silent: do not queue an import
	// title and do not transition the checkpoint to failed. Only the explicit
	// publish-history commands below may reread across that boundary.
	...
	automaticSourceProof := outboxQueueOptions{}
	if !transcriptImportRunIsExplicitHistory(importTurnID) {
		var proofErr error
		automaticSourceProof, proofErr = b.preflightAutomaticTranscriptImportSourceProof(ctx, session, local.FilePath, checkpointID)
		if proofErr != nil {
			if errors.Is(proofErr, errTranscriptAutomaticSourceProofUnavailable) {
				// Do not queue an import title for a source whose automatic boundary
				// cannot be authenticated. The blocked checkpoint is silent; the
				// operator-selected publish-history path remains available.
				return nil
			}
			return proofErr
		}
	...
	if err := b.queueOrSendOutboxChunks(ctx, session.ID, importTurnID, session.ChatID, kindPrefix+"-title", title, automaticSourceProof, opts.QueueOnly); err != nil {
	...
	if err != nil {
		if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
			// Automatic history remains silently blocked at its last trusted
			// boundary. Do not turn a source race/legacy proof gap into a failed
			// import or a repeated needs-attention card; explicit publish-history
			// is the operator-selected recovery boundary.
			return nil
		}
		_ = b.markTranscriptImportFailedWithID(ctx, session, local.FilePath, checkpointID)
		if isTranscriptCheckpointNotFoundError(err) {
	...
	if !result.Complete {
		return b.markTranscriptImportPausedAtWithProof(ctx, session, local.FilePath, result.LastRecordID, result.LastLine, result.LastOffset, checkpointID, importTurnID, kindPrefix, result.SourceProof)
	}
	return b.finishTranscriptImport(ctx, session, local, result, checkpointID, importTurnID, kindPrefix, opts)
}

// finishTranscriptImport makes the post-scan side effects recoverable as one
// durable phase. The cursor is advanced by the batcher while the checkpoint is
// still importing; only after subagent markers and the idempotent completion
// notice are queued do we clear CompletionPending and mark the import complete.
// A notification failure therefore cannot strand a fully imported transcript in
// a terminal failed state with no way for the background watcher to retry it.
func (b *Bridge) finishTranscriptImport(ctx context.Context, session Session, local codexhistory.Session, result transcriptImportResult, checkpointID string, importTurnID string, kindPrefix string, opts transcriptImportRunOptions) error {
	if err := b.markTranscriptImportCompletionPending(ctx, session, local.FilePath, checkpointID); err != nil {
		return err
	}
	...
	if err := b.queueOrSendOutboxChunks(ctx, session.ID, importTurnID, session.ChatID, kindPrefix+"-complete", complete, result.SourceProof, opts.QueueOnly); err != nil {
		return err
	}
	if err := b.markTranscriptImportCompleteFromResult(ctx, session, local.FilePath, result, checkpointID); err != nil {
		if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
			// The transcript rows are already durable, but their source boundary
			// changed before the completion CAS. Leave the checkpoint blocked and
			// let the next explicit history command choose the recovery boundary;
			// never turn this race into a failed notice loop.
			return nil
		}
		return err
	}
...
func (b *Bridge) finishExplicitTranscriptImport(ctx context.Context, session Session, local codexhistory.Session, result transcriptImportResult, checkpointID string, importTurnID string, kindPrefix string) error {
	if err := b.markTranscriptImportCompletionPending(ctx, session, local.FilePath, checkpointID); err != nil {
		return err
	}
	if err := b.importSubagentMarkersToTeamsWithOptions(ctx, session, local, importTurnID, transcriptImportRunOptions{
		SubagentCheckpointRoot: checkpointID,
	}); err != nil {
		return err
	}
	...
	if checkpoint, found, checkpointErr := b.store.ImportCheckpoint(ctx, checkpointID); checkpointErr != nil {
		return checkpointErr
	} else if found && executionAnchorActive(checkpoint.UnresolvedExecution) {
		body = "Import complete. History was imported, but the previous Codex execution is still unconfirmed. I did not clear the execution anchor or start/retry a new turn. Check the existing work; normal dispatch resumes only after explicit terminal ownership is confirmed."
	}
	if err := b.queueOrSendOutboxChunks(ctx, session.ID, importTurnID, session.ChatID, kindPrefix+"-complete", body, result.SourceProof, false); err != nil {
		return err
	}
	return b.markTranscriptImportCompleteFromResult(ctx, session, local.FilePath, result, checkpointID)
}
...
func (b *Bridge) publishWorkSessionHistory(ctx context.Context, session *Session) error {
	...
	if !result.Complete {
		if pauseErr := b.markTranscriptImportPausedAt(ctx, *session, local.FilePath, result.LastRecordID, result.LastLine, result.LastOffset, transcriptCheckpointID(session.ID), importTurnID, "sync"); pauseErr != nil {
			return pauseErr
		}
		return b.sendToChat(ctx, session.ChatID, "History import paused at the last complete transcript record because the local history is still being written or contains an incomplete record. No completion message was sent; run `helper publish-history` again after the file settles.")
	}
	if err := b.markTranscriptImportCompleteFromResult(ctx, *session, local.FilePath, result, transcriptCheckpointID(session.ID)); err != nil {
		return err
	}
	body := formatTranscriptImportCompleteMessage(result.Stats)
	if result.Stats.Imported == 0 && result.Stats.SkippedBackground == 0 {
...
func (b *Bridge) publishWorkSessionFullHistory(ctx context.Context, session *Session) error {
	...
	if complete, err := b.transcriptCheckpointAtSourceEOF(ctx, checkpointID, local.FilePath); err != nil {
		...
		return b.sendToChat(ctx, session.ChatID, "Full history import paused at the last complete transcript record because the local history is still being written or contains an incomplete record. No completion message was sent; run `helper publish-history full` again after the file settles.")
	}
...
func (b *Bridge) PublishSessionFullHistory(ctx context.Context, selector string, forceRunID string) (bool, error) {
	...
	if force := strings.TrimSpace(forceRunID); force != "" {
		checkpointID += ":force:" + shortStableID(force)
	}
	complete, err := b.transcriptCheckpointAtSourceEOF(ctx, checkpointID, local.FilePath)
	if err != nil {
		return false, err
	}
	if !complete {
...
func (b *Bridge) importTranscriptRecordsToTeams(ctx context.Context, session Session, filePath string, importTurnID string, kindPrefix string, checkpointID string, opts transcriptImportRunOptions) (transcriptImportResult, error) {
	...
	sourceProof := outboxQueueOptions{}
	if !transcriptImportRunIsExplicitHistory(importTurnID) {
		sourceProof = transcriptAutomaticImportProofQueueOptions(filePath, checkpoint, sourceSizeBeforeRead)
		if strings.TrimSpace(sourceProof.ExpectedSourceFingerprint) == "" ||
			!transcriptAutomaticImportSourceProofMatches(sourceProof) {
			block := b.blockAutomaticTranscriptSyncForSourceRewrite
			if strings.TrimSpace(sourceProof.ExpectedSourceFingerprint) == "" {
				block = func(blockCtx context.Context, blockSession Session, blockPath string, blockCheckpoint teamstore.ImportCheckpoint) error {
					return b.blockAutomaticTranscriptSync(blockCtx, blockSession, blockPath, blockCheckpoint, 0)
				}
			}
			if blockErr := block(ctx, session, filePath, checkpoint); blockErr != nil {
				return transcriptImportResult{}, blockErr
			}
			return transcriptImportResult{}, errTranscriptAutomaticSourceProofUnavailable
		}
	}
	if checkpoint.LastRecordID == "" {
		...
		for attempt := 0; attempt < 2; attempt++ {
			if allowAmbiguousImport {
				// Explicit publish-history is an operator-selected full read. Keep
				// it independent of the bounded automatic polling scanner so a
				// large backlog remains recoverable without reintroducing an
				// unbounded hot path for ordinary sync.
			...
			recovered, recoverErr := b.recoverFailedTranscriptCheckpoint(ctx, session, codexhistory.Session{
				...
				ProjectPath: session.Cwd,
				FilePath:    filePath,
			}, transcriptImportRunIsExplicitHistory(importTurnID))
			if recoverErr != nil {
	...
	if transcriptHasDiagnostic(transcript, "source_rewritten") || transcriptHasDiagnostic(transcript, "checkpoint_provenance_mismatch") {
		if !allowAmbiguousImport {
			if blockErr := b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, session, filePath, checkpoint); blockErr != nil {
				return transcriptImportResult{}, blockErr
			}
			// This is a source-boundary race, not a missing checkpoint selected by
			// the operator. Keep automatic history silent; the outer import/resume
			// paths only publish a needs-attention notice for the explicit
	...
	if (transcript.UnresolvedContinuation || transcript.PendingContinuation || transcript.TranscriptQuarantine != nil) && allowAmbiguousImport {
		...
		// through the explicit import path and keep the ownership marker intact.
		explicit, readErr := ReadSessionTranscriptSince(filePath, checkpoint.LastRecordID)
		if readErr != nil {
			return transcriptImportResult{}, readErr
		}
		if transcriptHasDiagnostic(explicit, "checkpoint_not_found") {
			return transcriptImportResult{}, errTranscriptCheckpointNotFound
		}
	...
	if transcript.PendingContinuation {
		if allowAmbiguousImport {
			// The explicit reread above is now the selected import boundary.
			transcript.PendingContinuation = false
		} else {
			// A root-shaped task marker is not yet an anchor: a later visible user
			// prompt may prove a normal new turn. Do not import or advance this
			// checkpoint until that ownership evidence arrives.
			return transcriptImportResult{Complete: false}, nil
		}
	}
	// The proof was captured before opening/parsing the transcript. Revalidate
	// it after the read as well: an atomic replacement during parsing must not
	// bind records from the old descriptor to the new pathname.
	if strings.TrimSpace(sourceProof.ExpectedSourceFingerprint) != "" &&
		!transcriptAutomaticImportSourceProofMatches(sourceProof) {
		if blockErr := b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, session, filePath, checkpoint); blockErr != nil {
			return transcriptImportResult{}, blockErr
	...
	batcher := newTranscriptImportBatcher(b, session, filePath, importTurnID, kindPrefix, checkpointID, sourceProof, opts)
	...
	for i, record := range transcript.Records {
		...
		if !allowAmbiguousImport && sourceProof.ExpectedSourceReadRangeKnown && checkpointOffset > sourceProof.ExpectedSourceReadEndOffset {
			// The file grew while it was being read. Do not queue a record that was
			// outside the pre-read snapshot; the next bounded pass will import it
			// under a fresh read proof.
			return withSourceBoundary(transcriptImportResult{
	...
	if len(transcript.Records) == 0 {
		return withSourceBoundary(transcriptImportResult{Stats: stats, Complete: true}), nil
	}
...
func newTranscriptImportBatcher(b *Bridge, session Session, filePath string, importTurnID string, kindPrefix string, checkpointID string, sourceProof outboxQueueOptions, opts transcriptImportRunOptions) *transcriptImportBatcher {
...
func (b *transcriptImportBatcher) add(ctx context.Context, record transcriptImportBatchRecord) error {
	...
	if record.Record.Kind == TranscriptKindAssistant && len(parseTrustedTeamsMath(record.Body).Spans) > 0 {
		...
		if err := b.bridge.queueOrSendTranscriptDeliveryChunksWithOptions(ctx, b.session, local, record.Record, record.Record.SourceLine, record.Record.SourceOffset, record.Kind, record.Body, b.sourceProof, b.importTurnID, b.checkpointID, true, b.queueOnly, b.deliveryNS); err != nil {
			return err
		}
		b.queuedBatches++
		return nil
...
func (b *transcriptImportBatcher) flushCheckpoint(ctx context.Context) error {
	...
	err := b.bridge.recordTranscriptCheckpointProgressDetailedWithSourceProof(ctx, b.session, b.filePath, checkpoint.Key, checkpoint.SourceLine, checkpoint.SourceOffset, b.checkpointID, b.sourceProof)
	if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
		return b.blockForSourceProofFailure(ctx)
	}
	return err
}

func (b *transcriptImportBatcher) blockForSourceProofFailure(ctx context.Context) error {
	if b == nil || b.bridge == nil {
		return errTranscriptAutomaticSourceProofUnavailable
	}
	previous := teamstore.ImportCheckpoint{
		ID: b.checkpointID, SessionID: b.session.ID, SourcePath: b.filePath,
	}
	if err := b.bridge.blockAutomaticTranscriptSyncForSourceRewrite(ctx, b.session, b.filePath, previous); err != nil {
		return err
	}
	return errTranscriptAutomaticSourceProofUnavailable
}
...
func (b *transcriptImportBatcher) flush(ctx context.Context) error {
	...
	// Validate the same proof immediately before queueing.  The checkpoint
	// writer validates it again after the queue operation, closing the race in
	// which the transcript is atomically replaced between those two writes.
	if strings.TrimSpace(b.sourceProof.ExpectedSourceFingerprint) != "" &&
		!transcriptAutomaticImportSourceProofMatches(b.sourceProof) {
		return b.blockForSourceProofFailure(ctx)
	}
	if err := b.bridge.queueOrSendTranscriptImportBatch(ctx, b.session, b.filePath, b.checkpointID, b.importTurnID, kind, html, first.Record, last.Record, b.sourceProof, b.queueOnly, b.deliveryNS); err != nil {
		return err
	}
	b.queuedBatches++
...
func (b *Bridge) queueAndSendTranscriptImportBatch(ctx context.Context, session Session, sourcePath string, checkpointID string, turnID string, kind string, html string, first TranscriptRecord, last TranscriptRecord) error {
	return b.queueOrSendTranscriptImportBatch(ctx, session, sourcePath, checkpointID, turnID, kind, html, first, last, outboxQueueOptions{}, false, "")
}

func (b *Bridge) queueOrSendTranscriptImportBatch(ctx context.Context, session Session, sourcePath string, checkpointID string, turnID string, kind string, html string, first TranscriptRecord, last TranscriptRecord, sourceProof outboxQueueOptions, queueOnly bool, deliveryNamespace string) error {
	...
	applyTranscriptSourceProofToOutbox(&msg, sourceProof)
	queued, _, _, err := b.store.QueueTranscriptDeliveryOutbox(ctx, teamstore.TranscriptDeliveryQueueRequest{
		...
		Checkpoint: teamstore.ImportCheckpoint{
			ID:                checkpointID,
			SessionID:         session.ID,
			SourcePath:        sourceProof.ExpectedSourcePath,
			SourceFingerprint: sourceProof.ExpectedSourceFingerprint,
			LastOffset:        sourceProof.ExpectedSourceOffset,
			LastOffsetKnown:   sourceProof.ExpectedSourceOffsetKnown,
		},
	})
	if err != nil || queueOnly || queued.ID == "" || queued.Status == teamstore.OutboxStatusSent {
		return err
	}
	return b.flushPendingOutboxFilteredWithOptions(ctx, "", turnID, queued.TeamsChatID, outboxFlushOptions{
		AllowAmbiguousRetry:           outboxTurnAllowsAmbiguousRetry(turnID),
		AllowProtectedAmbiguousBypass: outboxTurnAllowsProtectedAmbiguousBypass(turnID),
		SkipUnresolvedTranscript:      true,
	})
...
func (b *Bridge) transcriptCheckpointAtSourceEOF(ctx context.Context, checkpointID string, sourcePath string) (bool, error) {
	...
	return checkpoint.LastOffset >= sourceSize && checkpoint.SourceSize == sourceSize && checkpoint.SourceModTime.Equal(info.ModTime()) &&
		strings.TrimSpace(checkpoint.SourceFingerprint) != "" && checkpoint.SourceFingerprint == transcriptCheckpointSourceFingerprint(sourcePath, checkpoint.LastOffset), nil
}
...
func (b *Bridge) transcriptHasNewRecords(ctx context.Context, session Session, local codexhistory.Session) (bool, error) {
	...
	if checkpoint.Status == importCheckpointStatusFailed {
		...
		recovered, err := b.recoverFailedTranscriptCheckpoint(ctx, session, local, false)
		if err != nil {
			return false, err
		}
		if recovered {
	...
	if transcriptHasDiagnostic(transcript, "checkpoint_not_found") {
		// The saved record key disappeared even though the bounded source proof
		// passed.  Treat this as a source rewrite/corrupt boundary and fence it
		// without emitting the legacy attention card.
...
func (b *Bridge) prepareLocalCodexBeforeTeamsTurn(ctx context.Context, session *Session) (localCodexBeforeTeamsGate, error) {
	return b.prepareLocalCodexBeforeTeamsTurnWithQueueState(ctx, session, nil)
}

func (b *Bridge) prepareLocalCodexBeforeTeamsTurnWithQueueState(ctx context.Context, session *Session, knownTurns *sessionTurnQueueState) (localCodexBeforeTeamsGate, error) {
	if b == nil || session == nil {
	...
	// The linked transcript is an observation channel, not proof that a process
	// still owns this session. In particular, a legacy checkpoint can describe a
	// perfectly ordinary old tail as importing/blocked/failed, and an appended
	// tool or user record can look active after the CLI has already exited. None
	// of those states may prevent a new Teams turn from being accepted.
	//
	// Actual helper-owned execution is represented by the durable running-turn
	// row and is handled by the queue path below. Unknown app-server ownership is
	// handled by sessionExecutionOwnershipUnresolved above. Keeping this function
	// free of transcript discovery also makes the admission decision O(1) with
	// respect to local history size.
	if knownTurns != nil {
		if knownTurns.Running {
			return localCodexBeforeTeamsGate{
				Block:   true,
...
func (b *Bridge) advanceRecentCompletedTeamsTranscriptTailWithParentFence(ctx context.Context, session Session, local codexhistory.Session, parentFenceSessionID string) (bool, error) {
	...
	if transcriptHasDiagnostic(transcript, "source_rewritten") || transcriptHasDiagnostic(transcript, "checkpoint_provenance_mismatch") {
		return false, b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, session, local.FilePath, checkpoint)
	}
	teamsOriginHashes := teamsOriginTextHashes(state, session.ID)
	known := newKnownTranscriptOutboxDedupeState(state, session.ID, checkpoint.UpdatedAt)
	dedupe := newTranscriptDedupeState()
...
func transcriptDeliveryRecordWithNamespace(session Session, local codexhistory.Session, record TranscriptRecord, kind string, body string, deliveryNamespace string) teamstore.TranscriptDeliveryRecord {
	...
	codexTurnID := ""
	if record.TurnIDExplicit {
		codexTurnID = strings.TrimSpace(record.TurnID)
	}
...
func transcriptImportBatchDeliveryRecord(session Session, sourcePath string, checkpointID string, importTurnID string, kind string, html string, first TranscriptRecord, last TranscriptRecord, deliveryNamespace string) teamstore.TranscriptDeliveryRecord {
	...
	codexTurnID := ""
	if first.TurnIDExplicit {
		codexTurnID = strings.TrimSpace(first.TurnID)
	} else if last.TurnIDExplicit {
		codexTurnID = strings.TrimSpace(last.TurnID)
	}
	return teamstore.TranscriptDeliveryRecord{
		ID:             "transcript-delivery:" + strings.TrimSpace(session.ID) + ":" + hex.EncodeToString(sum[:])[:24],
		SessionID:      strings.TrimSpace(session.ID),
...
// recordSkippedTranscriptDeliveryWithSourceProof records the dedupe/quarantine
// fact first, then advances the linked-transcript cursor through the same
// source-proof CAS used by visible deliveries. A skipped row must not carry a
// freshly synthesized checkpoint by itself: doing so would let a same-size
// rewrite of the unread tail advance the cursor without checking the bytes
...
func (b *Bridge) syncLinkedTranscriptsWithDiscovery(ctx context.Context, forceDiscovery bool, now time.Time) error {
	...
	// Checkpoints are needed to locate each linked source.  Do not decode every
	// session's running/interrupted turns yet: a trusted unchanged EOF is a
	// side-effect-free no-op and can safely avoid the ownership query entirely.
	checkpoints, err := b.store.ImportCheckpointsForSessions(ctx, candidateSessionIDs)
	if err != nil {
		return err
	}
	probeSessionIDs := make([]string, 0, len(candidateSessionIDs))
	// Reuse the local source stat gathered while deciding whether an ownership
	// probe is needed. The strict idle/source-prefix checks below still stat and
	// reopen the source when a side effect is possible; this cache only avoids a
	...
	for _, session := range b.reg.Sessions {
		...
		local, localOK := linkedTranscriptLocalFromCheckpoint(session, checkpoint)
		localKnown[session.ID] = true
		if localOK {
			localsBySession[session.ID] = local
		}
		if !hasCheckpoint || !localOK || !linkedTranscriptCheckpointIdleUnchanged(local.FilePath, checkpoint) || executionAnchorActive(checkpoint.UnresolvedExecution) {
			probeSessionIDs = append(probeSessionIDs, session.ID)
		}
	}
	executionSnapshot, err := b.store.LinkedTranscriptExecutionSnapshot(ctx, probeSessionIDs)
	if err != nil {
		return err
	}
	...
	loadSessionState := func(session Session, checkpoint teamstore.ImportCheckpoint) (teamstore.State, error) {
		...
		// one richer snapshot for all importing sessions in this sync cycle so a
...
func (b *Bridge) queueActiveTurnTranscriptStatusBeforeFinal(ctx context.Context, session *Session, turn teamstore.Turn) (activeTurnTranscriptPreparation, error) {
	...
	if found && strings.TrimSpace(checkpoint.SessionID) == "" {
		return preparation, fmt.Errorf("%w: checkpoint %q has no owning session", teamstore.ErrSessionStateProvenanceMismatch, checkpointID)
	}
	if found && strings.TrimSpace(checkpoint.ID) != checkpointID {
		return preparation, fmt.Errorf("%w: checkpoint row id %q does not match requested %q", teamstore.ErrSessionStateProvenanceMismatch, checkpoint.ID, checkpointID)
	}
	...
	// Active-turn pre-final backfill is an automatic linked-transcript read,
	// even though it is entered from the completion path rather than the normal
	// history-watch loop.  Do not let the reader's legacy compatibility fallback
	...
	if transcriptHasDiagnostic(transcript, "source_rewritten") || transcriptHasDiagnostic(transcript, "checkpoint_provenance_mismatch") {
		return preparation, b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, *session, local.FilePath, checkpoint)
	}
	if transcriptHasDiagnostic(transcript, "checkpoint_not_found") || len(transcript.Records) == 0 {
		return preparation, nil
	}
	...
	flushCheckpoint := func() error {
		...
		pendingCheckpoint = transcriptImportCheckpointRecord{}
		err := b.recordTranscriptCheckpointProgressDetailedWithSourceProof(ctx, *session, local.FilePath, pending.Key, pending.SourceLine, pending.SourceOffset, checkpointID, sourceProof)
		if errors.Is(err, teamstore.ErrUnresolvedExecution) && hasFinalCheckpoint {
			// An exact terminal callback may own the active anchor, but the
			// intermediate status cursor cannot be committed in a separate
			// transaction. Defer the whole prefix to the final completion CAS.
			deferredFinalCheckpoint = true
			return nil
		}
		return err
	}
	...
	for i, record := range transcript.Records {
		...
		if hasFinalCheckpoint && i == finalIndex {
			...
			if checkpointAdvanceAllowed || deferredFinalCheckpoint {
		...
		case TranscriptKindStatus, TranscriptKindCompact:
			if err := b.queueOrSendTranscriptDeliveryChunksWithOptions(ctx, *session, local, record, checkpointLine, checkpointOffset, kind, body, transcriptSourceProofQueueOptions(checkpoint), turn.ID, "", false, true, ""); err != nil {
				return preparation, err
			}
			preparation.Queued++
			rememberCheckpoint(record, checkpointLine, checkpointOffset)
...
func (b *Bridge) queueRunningTurnTranscriptBackfill(ctx context.Context, sessionID string, turnID string, chatID string, expectedThreadID string, maxRecords int) (int, error) {
	if b == nil || b.store == nil || strings.TrimSpace(sessionID) == "" || strings.TrimSpace(turnID) == "" {
		return 0, nil
	}
	if maxRecords <= 0 {
	...
	sessionCopy := *session
	// The caller may have captured a pre-recreate chat ID. Durable session state
	// is authoritative; use the caller value only for legacy rows without one.
	sessionCopy.ChatID = firstNonEmptyString(sessionCopy.ChatID, chatID)
	if strings.TrimSpace(sessionCopy.ChatID) == "" {
		return 0, nil
	}
	...
	if transcriptHasDiagnostic(transcript, "source_rewritten") || transcriptHasDiagnostic(transcript, "checkpoint_provenance_mismatch") {
		return 0, b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, sessionCopy, local.FilePath, checkpoint)
	}
	...
	flushInternal := func() error {
		...
		return b.recordTranscriptCheckpointProgressDetailedWithSourceProof(ctx, sessionCopy, local.FilePath, checkpoint.Key, checkpoint.SourceLine, checkpoint.SourceOffset, checkpointID, sourceProof)
	}
	for i, record := range transcript.Records {
		checkpointLine, checkpointOffset := transcriptCheckpointPositionForRecord(transcript.Records, i)
		record.SourceLine = checkpointLine
		...
		case TranscriptKindStatus, TranscriptKindCompact:
			kind := transcriptRecordOutboxKind("codex", record, i+1)
			if err := b.queueOrSendTranscriptDeliveryChunksWithOptions(ctx, sessionCopy, local, record, checkpointLine, checkpointOffset, kind, body, transcriptSourceProofQueueOptions(checkpoint), turn.ID, checkpointID, true, true, ""); err != nil {
				return queued, err
			}
...
func (b *Bridge) syncSessionTranscriptFromSnapshot(ctx context.Context, session Session, local codexhistory.Session, state teamstore.State, checkpoint teamstore.ImportCheckpoint, hasCheckpoint bool) (err error) {
	...
	if hasCheckpoint {
		...
		case importCheckpointStatusImporting:
			if checkpoint.CompletionPending || transcriptImportCheckpointIsOrphaned(state, checkpoint) {
	...
	if hasCheckpoint && checkpoint.Status == importCheckpointStatusComplete &&
		strings.TrimSpace(checkpoint.LastRecordID) != "" && checkpoint.LastOffset <= 0 {
		// A legacy complete row with no usable byte cursor still gets the old
		// stale-cursor diagnostic. If its record is genuinely absent, mark it
		// failed so explicit recovery can repair it; only an existing record
		...
		if sourcePath != "" {
			...
			if !found {
				return b.markTranscriptImportFailedWithParentFence(ctx, session, sourcePath, checkpointID, session.ID)
			}
		}
	}
	if hasCheckpoint && linkedTranscriptCheckpointNeedsAutomaticSourceProof(checkpoint, local.FilePath) {
	...
	refreshCheckpoint := func() error {
		latest, found, err := b.store.ImportCheckpoint(ctx, checkpointID)
		if err != nil {
			return err
		}
		if found {
	...
	persistConsumedProgress := func() error {
		...
		observation := transcriptCheckpointObservationForTranscript(transcript, &checkpoint, hasCheckpoint)
		return b.recordTranscriptCheckpointObservation(
			ctx, session, local.FilePath, transcript.Consumed.RecordID, transcript.Consumed.Line,
			transcript.Consumed.Offset, checkpointID, sourceProof, session.ID, observation,
		)
	}
	...
	if transcript.UnresolvedContinuation && len(transcript.Records) == 0 {
		// There is no safe prefix to deliver before the execution boundary. This
		// is still transcript evidence, not proof that an app-server process owns
		// the chat. Commit the physical cursor plus the bounded pending range
		// before retaining the silent history-only quarantine; otherwise the same
		// marker is scanned forever and s512 never reaches later evidence.
	...
	sent := 0
	processedAllTranscriptRecords := false
	...
	flushPendingCheckpoint := func() error {
		...
		var err error
		if boundary == nil {
			err = b.recordTranscriptCheckpointObservation(ctx, session, local.FilePath, pending.Key, pending.SourceLine, pending.SourceOffset, checkpointID, sourceProof, session.ID, observation)
		} else {
			err = b.recordTranscriptCheckpointObservation(ctx, session, local.FilePath, pending.Key, pending.SourceLine, pending.SourceOffset, checkpointID, sourceProof, session.ID, observation, *boundary)
		}
		if err != nil {
			return err
		}
		// A scan may flush several bounded visible prefixes in one cycle. Refresh
		// the optimistic snapshot after each successful CAS so the next prefix is
		// compared with the cursor this reader actually committed, not the stale
		// snapshot from the beginning of the scan.
		if latest, found, loadErr := b.store.ImportCheckpoint(ctx, checkpointID); loadErr != nil {
			return loadErr
		} else if found {
			checkpoint = latest
			hasCheckpoint = true
	...
	persistDeferredHistoryQuarantine := func() error {
		current, found, err := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(session.ID))
		if err != nil {
			return err
		}
		if !found {
		...
		return b.persistHistoryOnlyContinuationQuarantine(ctx, session, local, current, transcript)
	}
	finishSafePrefix := func() error {
		if err := flushPendingCheckpoint(); err != nil {
			return err
		}
		if checkpointNeedsRefresh {
			if err := refreshCheckpoint(); err != nil {
				return err
			}
			checkpointNeedsRefresh = false
		}
		if len(transcript.Records) > 0 && !processedAllTranscriptRecords {
			// The scanner may have read a complete bounded prefix containing
			// more records than this cycle is allowed to deliver.  Consumed is
			// the scanner's physical end, not proof that every visible record
			// before it was delivered; never jump over the unsent suffix.
			if deferUnresolvedContinuation {
				return persistDeferredHistoryQuarantine()
			}
			return nil
		}
		if err := persistConsumedProgress(); err != nil {
		...
		if deferUnresolvedContinuation {
			// The scanner found an ambiguous history boundary after the safe
			// prefix. Persist only a history quarantine; never promote this
			// transcript observation into an execution owner or a live queue gate.
			return persistDeferredHistoryQuarantine()
		}
		return nil
	}
...
// checkpoints written before terminal-boundary provenance was persisted. It
// parses complete JSON records only; matching text inside an ordinary message
// must never manufacture an execution boundary. The probe remains bounded and
// conservative: a positive typed terminal/final record lets the incremental
// scanner quarantine a child continuation, while an absent record preserves the
// existing legacy-cursor rules. A bare assistant final is not treated as a
// terminal event here because linked checkpoints may intentionally stop after
// a progress record before the final callback is persisted.
func linkedCheckpointPrefixHasTerminalBoundary(sourcePath string, offset int64) bool {
	return linkedCheckpointPrefixHasTerminalBoundaryForThread(sourcePath, offset, "")
}

func linkedCheckpointPrefixHasTerminalBoundaryForThread(sourcePath string, offset int64, expectedThreadID string) bool {
	sourcePath = strings.TrimSpace(sourcePath)
	if sourcePath == "" || offset <= 0 {
		return false
	}
	const windowBytes int64 = 64 * 1024
	start := offset - windowBytes
	if start < 0 {
		start = 0
	}
	...
	for i, line := range lines {
		// The bounded window can start in the middle of a record. Never parse
		// that fragment as a boundary, and never parse an unterminated final
		// fragment at the cursor.
		if (i == 0 && start > 0) || (i == len(lines)-1 && !bytes.HasSuffix(window, []byte{'\n'})) {
			continue
		}
		...
		hints := historyTieredLineHints(line)
		if !hints.MayTerminalEvent {
			continue
		}
		signals := historyTieredLineSignalsWithHints(line, hints)
		// A type name alone is not ownership proof. Legacy checkpoints are only
		// allowed to establish a terminal boundary with an explicit turn ID. A
		// linked source whose session/thread identity is already trusted may omit
		// thread_id on the terminal event; when it does provide one, it must agree
		// with that source identity. The standalone probe (without source context)
		// remains strict and requires both IDs.
		threadID := strings.TrimSpace(signals.ThreadID)
		expectedThreadID = strings.TrimSpace(expectedThreadID)
		threadProvenance := expectedThreadID != "" && (threadID == "" || threadID == expectedThreadID) || expectedThreadID == "" && threadID != ""
		if (signals.TurnCompleted || signals.TerminalFailed) && strings.TrimSpace(signals.TurnID) != "" && threadProvenance {
			return true
		}
	}
	return false
...
func (b *Bridge) readLinkedTranscriptDelta(filePath string, checkpoint teamstore.ImportCheckpoint, sessionID string, threadID string) (Transcript, error) {
	...
	if strings.TrimSpace(checkpoint.SourcePath) != "" && cleanComparablePath(checkpoint.SourcePath) != cleanComparablePath(filePath) {
	...
	if linkedCheckpointOffsetKnown(checkpoint) && strings.TrimSpace(checkpoint.SourcePath) != "" {
		if ok := linkedCheckpointPrefixMatches(filePath, checkpoint); !ok {
			return Transcript{SourceName: filePath, Diagnostics: []TranscriptDiagnostic{{
				Kind:    "source_rewritten",
				Message: "transcript source prefix no longer matches the trusted checkpoint",
			}}}, nil
		}
		previous := historyTieredFileState{
			Path:                      filePath,
			...
			TerminalBoundarySeen:      checkpoint.TerminalBoundarySeen || checkpoint.TerminalBoundary != nil || linkedCheckpointPrefixHasTerminalBoundaryForThread(filePath, checkpoint.LastOffset, threadID),
			TerminalBoundaryLine:      checkpoint.TerminalBoundaryLine,
			TerminalBoundary:          checkpoint.TerminalBoundary,
			LastFinalID:               firstNonEmptyString(checkpoint.LastFinalID, checkpoint.LastRecordID),
			LastFinalLine:             checkpoint.LastFinalLine,
		...
		// The prefix check above protects the scanner's starting cursor, but the
		// pathname can be replaced while the bounded tail is being read. Recheck
		// the same source identity and prefix after the scan before exposing any
		// records to the bridge. The final checkpoint/outbox CAS remains the last
		// barrier; this second check closes the common scan/reopen TOCTOU window.
		if !linkedCheckpointPrefixMatches(filePath, checkpoint) {
			return Transcript{SourceName: filePath, Diagnostics: []TranscriptDiagnostic{{
				Kind:    "source_rewritten",
		...
		if result.Incomplete {
			...
			// the same pass look complete even though the ownership boundary is
			// still changing.
		...
		if result.Truncated {
			...
			// fall back to a full parser here: that would make an old terminal
			// boundary look like a safe prefix and could publish a new child
			// answer under the wrong owner.
			return Transcript{
				SourceName: filePath,
				Diagnostics: []TranscriptDiagnostic{{
					Kind:    "source_rewritten",
					Message: "transcript source was replaced or truncated after the checkpoint",
				}},
			}, nil
		}
	...
	// Legacy checkpoints may have a record key but no byte position (or may
	// point at a path that was rewritten). Do not fall back to the permissive
	...
	if result.Truncated {
		return Transcript{SourceName: filePath, Diagnostics: []TranscriptDiagnostic{{
			Kind:    "source_rewritten",
			Message: "transcript source was replaced or truncated before a legacy checkpoint position was known",
		}}}, nil
	}
	if result.Incomplete {
		// A legacy checkpoint has no byte cursor, so the complete prefix must
		// prove that the saved record is present before any suffix is returned.
		// If the checkpoint is only present in the unterminated record (or the
		// source was rewritten), returning the whole prefix would guess an import
		// boundary and could publish an unrelated answer.
...
func withHistoryTieredReadProof(transcript Transcript, result historyTieredTailResult) Transcript {
	...
	transcript.ContextGap = result.State.ContextGap
	transcript.TerminalBoundary = result.State.TerminalBoundary
	if !result.Incomplete && !result.Truncated {
		...
		if result.State.UnresolvedContinuation {
			// The physical scanner has already consumed complete records through
			// the current bounded snapshot, but the suffix is not publishable.
			// Persist that physical progress with a bounded semantic range instead
			// of rewinding to the marker and livelocking forever.
			if pending := historyTieredPendingHistoryRange(result.State, result.State.UnresolvedContinuationOffset, result.State.Offset, "unresolved_continuation"); pending != nil {
		...
		if result.State.TranscriptQuarantine != nil && !historyTieredQuarantineIsContextGap(result.State.TranscriptQuarantine) && transcript.PendingHistoryRange == nil {
			...
			end := result.State.TranscriptQuarantine.ExclusiveEndOffset
			if end < result.State.Offset {
				end = result.State.Offset
			}
			transcript.PendingHistoryRange = historyTieredPendingHistoryRange(result.State, start, end, result.State.TranscriptQuarantine.Kind)
		}
		...
		if result.State.UnresolvedContinuation || (result.State.TranscriptQuarantine != nil && !historyTieredQuarantineIsContextGap(result.State.TranscriptQuarantine)) {
			if result.State.Offset > 0 && result.State.Offset >= consumedOffset {
				consumedOffset = result.State.Offset
				consumedLine = result.State.Line
				if consumedID == "" {
					consumedID = historyTieredConsumedRecordKey(result.State.Path, result.State.Line, 0, result.State.Offset)
				}
			}
		}
	...
	if result.ReadProofRangeKnown && strings.TrimSpace(result.ReadProofFingerprint) != "" {
		...
		transcript.SourceReadProofEndOffset = result.ReadProofEndOffset
		transcript.SourceReadProofRangeKnown = true
	}
	return transcript
}

func historyTieredPendingHistoryRange(state historyTieredFileState, startOffset, endOffset int64, kind string) *teamstore.HistoryPendingRange {
	if endOffset <= startOffset || startOffset < 0 || strings.TrimSpace(state.SourceGeneration) == "" {
		return nil
	}
	fingerprint := transcriptSourceRangeFingerprint(state.Path, startOffset, endOffset)
	if strings.TrimSpace(fingerprint) == "" {
		return nil
	}
	return &teamstore.HistoryPendingRange{
		SourceGeneration: strings.TrimSpace(state.SourceGeneration),
		RangeID:          historyTieredGapID(state.Path, state.SourceGeneration, kind, startOffset, endOffset),
		Kind:             strings.TrimSpace(firstNonEmptyString(kind, "history_ambiguity")),
		StartRecordID:    strings.TrimSpace(state.LastFinalID),
		StartLine:        state.UnresolvedContinuationLine,
		StartOffset:      startOffset,
		ExclusiveEnd:     endOffset,
...
func historyTieredFinalRecords(finals []historyTieredFinal) []TranscriptRecord {
	...
	records := make([]TranscriptRecord, 0, len(finals))
	for _, final := range finals {
		if final.Record.Text == "" {
			continue
		}
		records = append(records, final.Record)
	}
	return records
}
...
func filterTranscriptRecordsAfterCheckpoint(records []TranscriptRecord, afterKey string) []TranscriptRecord {
	...
	filtered, found := filterTranscriptRecordsAfterCheckpointStrict(records, afterKey)
	if !found {
		// Offset checkpoints already identify the exact byte boundary.  Preserve
		// the historical permissive fallback for those callers: a parser key can
		// legitimately differ from the legacy source ID after normalization, and
		// dropping the entire tail would lose visible history.  Legacy/no-offset
		// callers use the strict helper directly and fail closed instead.
		return records
	}
	return filtered
}

// filterTranscriptRecordsAfterCheckpointStrict differs from the historical
// permissive helper by reporting whether a non-empty checkpoint was actually
// found. Legacy checkpoints have no byte position and therefore must never
// fall back to treating the beginning of the source as the import boundary.
func filterTranscriptRecordsAfterCheckpointStrict(records []TranscriptRecord, afterKey string) ([]TranscriptRecord, bool) {
	afterKey = strings.TrimSpace(afterKey)
	if afterKey == "" || len(records) == 0 {
		return records, afterKey == ""
	}
	for i, record := range records {
		if record.DedupeKey == afterKey || record.ItemID == afterKey {
			return append([]TranscriptRecord(nil), records[i+1:]...), true
		}
	}
	return nil, false
}

func historyTieredBoundaryOffset(state historyTieredFileState) int64 {
	if state.UnresolvedContinuation {
		return state.UnresolvedContinuationOffset
	}
...
// filterTranscriptRecordsBeforeExecutionBoundary keeps ordinary progress that
// precedes an ambiguous task/turn marker while hiding the marker and every
// record after it. Live backfill can therefore preserve the existing
// pre-terminal progress behavior without leaking child commentary; explicit
// history import re-reads the full source separately when requested.
func filterTranscriptRecordsBeforeExecutionBoundary(records []TranscriptRecord, boundaryOffset int64) []TranscriptRecord {
	if boundaryOffset <= 0 || len(records) == 0 {
		return nil
	}
	filtered := make([]TranscriptRecord, 0, len(records))
	for _, record := range records {
		if record.SourceStartOffset >= boundaryOffset {
			break
		}
...
func linkedCheckpointFileUnchanged(filePath string, checkpoint teamstore.ImportCheckpoint) bool {
	if checkpoint.SourceSize <= 0 || checkpoint.SourceModTime.IsZero() || !linkedCheckpointOffsetKnown(checkpoint) {
		return false
	}
	if checkpoint.LastOffset != checkpoint.SourceSize || strings.TrimSpace(checkpoint.SourceFingerprint) == "" {
		return false
	}
	...
	// The pathname can be replaced while the bounded fingerprint is being
...
func linkedCheckpointOffsetKnown(checkpoint teamstore.ImportCheckpoint) bool {
	return checkpoint.LastOffsetKnown || checkpoint.LastOffset > 0
}

// linkedCheckpointPrefixMatches verifies the source identity before an
// incremental linked scan.  Size/mtime alone are only change hints: an
// in-place rewrite can preserve both and otherwise make a stale cursor look
// safe.  The bounded fingerprint is intentionally a local proof of the
// trusted window, not a claim that arbitrary bytes outside that window are
// immutable; a stronger source revision scheme would be a separate change.
func linkedCheckpointPrefixMatches(filePath string, checkpoint teamstore.ImportCheckpoint) bool {
	filePath = strings.TrimSpace(filePath)
	if filePath == "" || !linkedCheckpointOffsetKnown(checkpoint) || checkpoint.LastOffset < 0 {
		return false
	}
	// A zero-byte cursor is a valid empty prefix, but it cannot also carry a
	// saved record key. Treat that combination as a corrupt/legacy boundary
	// instead of letting the permissive record-key fallback publish an unrelated
	// replacement transcript from the beginning.
	if checkpoint.LastOffset == 0 && strings.TrimSpace(checkpoint.LastRecordID) != "" {
		return false
	}
	pathInfo, err := os.Stat(filePath)
	if err != nil || pathInfo.IsDir() || pathInfo.Size() < checkpoint.LastOffset {
		return false
	}
	f, err := os.Open(filePath)
	if err != nil {
	...
	// A known zero-byte cursor still has a source identity proof when it was
	// written by the current format.  Verify that proof before accepting an
	// atomic replacement; otherwise a new inode with a plausible suffix could
	// be mistaken for the same empty-prefix source.  The old no-fingerprint,
	// empty-source shape remains compatible and is handled conservatively.
	if checkpoint.LastOffset == 0 {
		if expected == "" {
			return checkpoint.SourceSize <= 0
		}
		actual := strings.TrimSpace(transcriptCheckpointSourceFingerprintFromReader(f, filePath, fdInfo.Size(), 0))
		if actual == "" || actual != expected {
			return false
		}
		postInfo, err := os.Stat(filePath)
	...
	if expected == "" {
		...
		// blocks a growing legacy EOF checkpoint before reaching this helper, so
		// keep the direct scanner's historical behavior for explicit callers. A
		// genuinely non-EOF legacy cursor has no safe incremental boundary and is
		// rejected here.
		return checkpoint.SourceSize <= 0 || checkpoint.SourceSize == checkpoint.LastOffset
	}
	actual := strings.TrimSpace(transcriptCheckpointSourceFingerprintFromReader(f, filePath, fdInfo.Size(), checkpoint.LastOffset))
	if actual == "" || actual != expected {
		return false
	}
	postInfo, err := os.Stat(filePath)
...
func linkedTranscriptCheckpointNeedsPositionBackfill(checkpoint teamstore.ImportCheckpoint, sourcePath string) bool {
	if checkpoint.Status != importCheckpointStatusComplete && checkpoint.Status != importCheckpointStatusBlocked {
		return false
	}
...
func (b *Bridge) backfillLinkedTranscriptCheckpointPositionWithParentFence(ctx context.Context, session Session, local codexhistory.Session, checkpoint teamstore.ImportCheckpoint, parentFenceSessionID string) (teamstore.ImportCheckpoint, bool, error) {
	...
	// Position recovery reconstructs only a cursor from a durable record ID. It
	// must not create a source fingerprint: without an old identity-bound proof,
	// the current file could be a same-ID replacement. The caller will fail
	// closed on the resulting no-fingerprint boundary and require explicit
	// history publication.
...
func linkedTranscriptCheckpointIdleUnchanged(filePath string, checkpoint teamstore.ImportCheckpoint) bool {
	...
	return linkedCheckpointIdleNoGrowth(filePath, checkpoint)
}

func (b *Bridge) linkedTranscriptCheckpointIdleUnchanged(filePath string, checkpoint teamstore.ImportCheckpoint) bool {
	if checkpoint.Status != importCheckpointStatusComplete || strings.TrimSpace(checkpoint.LastRecordID) == "" || transcriptImportCheckpointNeedsBudgetedResume(checkpoint, filePath) {
		return false
	}
	return linkedCheckpointIdleNoGrowth(filePath, checkpoint)
}

// linkedCheckpointIdleNoGrowth is deliberately weaker than
// linkedCheckpointFileUnchanged. It is only an idle/no-op proof: the cursor is
// already at the trusted EOF and this result must never advance a checkpoint,
// create an outbox row, or publish transcript data. In that case reading and
// hashing the old prefix on every poll provides no safety value. Any growth,
// shrink, or mtime change falls through to the strict SameFile/fingerprint
// checks before a cursor can advance or a tail can be delivered.
//
// In particular, an in-place same-size rewrite may go unnoticed by this
// no-growth probe. That is safe because this path performs no side effect; if
// the source later grows, linkedCheckpointPrefixMatches compares the persisted
// identity-bound fingerprint before scanning the new suffix.
func linkedCheckpointIdleNoGrowth(filePath string, checkpoint teamstore.ImportCheckpoint) bool {
	if checkpoint.SourceSize <= 0 || checkpoint.SourceModTime.IsZero() ||
		!linkedCheckpointOffsetKnown(checkpoint) || checkpoint.LastOffset != checkpoint.SourceSize ||
		strings.TrimSpace(checkpoint.SourceFingerprint) == "" {
		return false
	}
...
// upgrade-era checkpoint shape that has a complete EOF cursor but no bounded
// source fingerprint. It is retained as a named migration boundary so callers
// can fail closed and request explicit history publication; automatic sync must
// never manufacture an identity proof from the current file.
func linkedTranscriptCheckpointNeedsFingerprintBackfill(checkpoint teamstore.ImportCheckpoint, sourcePath string) bool {
	if checkpoint.Status != importCheckpointStatusComplete && checkpoint.Status != importCheckpointStatusBlocked || strings.TrimSpace(checkpoint.LastRecordID) == "" {
		return false
	}
	if strings.TrimSpace(checkpoint.SourceFingerprint) != "" || strings.TrimSpace(sourcePath) == "" {
		return false
	}
	if checkpoint.LastOffset <= 0 || checkpoint.SourceSize <= 0 || checkpoint.LastOffset != checkpoint.SourceSize || checkpoint.SourceModTime.IsZero() {
		return false
	}
	return true
}
...
func linkedTranscriptCheckpointNeedsConservativeLegacyBlock(checkpoint teamstore.ImportCheckpoint, sourcePath string) bool {
	if checkpoint.Status != importCheckpointStatusComplete && checkpoint.Status != importCheckpointStatusBlocked || strings.TrimSpace(checkpoint.LastRecordID) == "" ||
		strings.TrimSpace(checkpoint.SourceFingerprint) != "" || strings.TrimSpace(sourcePath) == "" || checkpoint.LastOffset <= 0 {
		return false
	}
	// SourceSize is the size recorded by the old checkpoint.  A non-EOF cursor
	// (including one whose size metadata is missing) is not a safe incremental
	// boundary without a content fingerprint.  Also check the current file so
	// an old EOF checkpoint that has since grown is blocked before the fast path.
	if checkpoint.SourceSize > checkpoint.LastOffset {
		return true
	}
	info, err := os.Stat(sourcePath)
	return err == nil && !info.IsDir() && info.Size() > checkpoint.LastOffset
}

// linkedTranscriptCheckpointNeedsAutomaticSourceProof is the common guard for
// automatic readers that can be entered outside syncSessionTranscript.  A
// checkpoint with a record key but no identity-bound source proof must never
// be allowed to manufacture a new fingerprint from the current file: doing so
// would turn a replaced/grown legacy source into an apparently trusted suffix.
// Explicit publish-history uses a separate full-read path and does not call
// this guard.
func linkedTranscriptCheckpointNeedsAutomaticSourceProof(checkpoint teamstore.ImportCheckpoint, sourcePath string) bool {
	if strings.TrimSpace(checkpoint.LastRecordID) == "" {
		return false
	}
	sourcePath = strings.TrimSpace(sourcePath)
	if sourcePath == "" || strings.TrimSpace(checkpoint.SourcePath) == "" ||
		cleanComparablePath(checkpoint.SourcePath) != cleanComparablePath(sourcePath) {
		return true
	}
	if strings.TrimSpace(checkpoint.SourceFingerprint) == "" || !checkpoint.LastOffsetKnown || !linkedCheckpointOffsetKnown(checkpoint) {
		return true
	}
	if checkpoint.LastOffset == 0 {
		return true
	}
	if checkpoint.LastOffset < 0 || checkpoint.SourceSize < checkpoint.LastOffset {
		return true
	}
	return linkedTranscriptCheckpointNeedsConservativeLegacyBlock(checkpoint, sourcePath) ||
...
func (b *Bridge) resumeBudgetedTranscriptImport(ctx context.Context, session Session, local codexhistory.Session, checkpoint teamstore.ImportCheckpoint) error {
	...
	if _, err := b.preflightAutomaticTranscriptImportSourceProof(ctx, session, sourcePath, checkpoint.ID); err != nil {
		if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
			return nil
		}
		return err
	...
	if !result.Complete {
		return b.markTranscriptImportPausedAtWithProof(ctx, session, sourcePath, result.LastRecordID, result.LastLine, result.LastOffset, checkpointID, importTurnID, kindPrefix, result.SourceProof)
	}
	resumedLocal := local
	resumedLocal.FilePath = sourcePath
	return b.finishTranscriptImport(ctx, session, resumedLocal, result, checkpointID, importTurnID, kindPrefix, opts)
}
...
func (b *Bridge) resumeInterruptedTranscriptImport(ctx context.Context, session Session, local codexhistory.Session, checkpoint teamstore.ImportCheckpoint) error {
	...
	kindPrefix := strings.TrimSpace(firstNonEmptyString(checkpoint.KindPrefix, "import"))
	checkpointID := strings.TrimSpace(firstNonEmptyString(checkpoint.ID, transcriptCheckpointID(session.ID)))
	explicit := transcriptImportRunIsExplicitHistory(importTurnID)
	if !explicit {
		if _, err := b.preflightAutomaticTranscriptImportSourceProof(ctx, session, sourcePath, checkpoint.ID); err != nil {
			if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
				return nil
	...
	if err != nil {
		if !explicit && errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
			return nil
		}
		_ = b.markTranscriptImportFailedWithID(ctx, session, sourcePath, checkpointID)
		if isTranscriptCheckpointNotFoundError(err) {
	...
	resumedLocal.FilePath = sourcePath
	if explicit {
		if err := b.finishExplicitTranscriptImport(ctx, session, resumedLocal, result, checkpointID, importTurnID, kindPrefix); err != nil {
			return err
		}
		return b.processQueuedTurns(ctx)
	}
	if err := b.finishTranscriptImport(ctx, session, resumedLocal, result, checkpointID, importTurnID, kindPrefix, transcriptImportRunOptions{
		SubagentCheckpointRoot: checkpointID,
	}); err != nil {
		if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
			return nil
...
func (b *Bridge) recoverFailedTranscriptCheckpoint(ctx context.Context, session Session, local codexhistory.Session, allowExplicitBoundary bool) (bool, error) {
	if b == nil || b.store == nil {
		return false, nil
	}
	state, err := b.store.Load(ctx)
	...
	if strings.TrimSpace(checkpoint.LastRecordID) != "" {
		...
		if !transcriptHasDiagnostic(transcript, "checkpoint_not_found") {
			if !allowExplicitBoundary {
				// A suffix proves only that the source has moved past the saved
				// cursor. It does not prove that those records reached Teams. An
				// automatic failed-import repair must not advance over them.
				return false, nil
			}
			// The key is still present. Make the failed checkpoint readable by
			// the explicit importer, but keep the cursor at the last durable
			// boundary. Advancing to the suffix end here used to silently skip
			// records whose Graph delivery had never been confirmed; the explicit
			// import can now re-read and deduplicate the suffix normally.
			return true, b.markFailedTranscriptCheckpointReady(ctx, session, checkpoint)
		}
	}
	if !allowExplicitBoundary {
		// Source-line recovery is an operator-selected boundary only. The
		// background watcher remains silent and leaves the failed cursor for
		// `helper publish-history` instead of guessing a new import position.
		return false, nil
	}
	// The saved key is gone. A line number is not a delivery ledger: after an
	// in-place rewrite, a different record can occupy the same line and the
	// old recovery code would advance past it as if it had already been sent.
	// Explicit publish-history is allowed to choose a new boundary, so choose
	// the conservative one—re-read from the beginning and let the existing
	// delivery ledger/idempotent outbox dedupe already-visible records.
	return true, b.resetFailedTranscriptCheckpointForExplicitImport(ctx, session, checkpoint, sourcePath)
}
...
func (b *Bridge) resetFailedTranscriptCheckpointForExplicitImport(ctx context.Context, session Session, checkpoint teamstore.ImportCheckpoint, sourcePath string) error {
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(current teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		current.PendingHistoryRange = nil
		current.ContextGap = nil
		current.CompletionPending = false
		current.Status = importCheckpointStatusComplete
		current.UpdatedAt = now
		return current, true, nil
...
// unresolvedAmbiguousCodexTurn identifies an interrupted Teams request whose
// Codex execution was not confirmed to have stopped, including helper restart
// and lost-context recovery paths. Durable turns are observations of the
// helper's state, not app-server ownership proof: a later Completed/Failed
// turn must never clear this ambiguity. The anchor is cleared only by the
// explicit app-server fence/terminal paths.
func unresolvedAmbiguousCodexTurn(state teamstore.State, session Session) (teamstore.Turn, bool) {
	sessionID := strings.TrimSpace(session.ID)
	if sessionID == "" {
		return teamstore.Turn{}, false
	}
	threadID := strings.TrimSpace(session.CodexThreadID)
	var candidates []teamstore.Turn
	for _, turn := range state.Turns {
		if strings.TrimSpace(turn.SessionID) != sessionID || turn.Status != teamstore.TurnStatusInterrupted {
			continue
		}
		if !isUnresolvedAmbiguousCodexRecoveryReason(turn.RecoveryReason) {
			continue
		}
		if threadID != "" && strings.TrimSpace(turn.CodexThreadID) != "" && strings.TrimSpace(turn.CodexThreadID) != threadID {
			continue
		}
	...
	sort.SliceStable(candidates, func(i, j int) bool {
		left := executionTurnBoundaryTime(candidates[i])
		right := executionTurnBoundaryTime(candidates[j])
		if !left.Equal(right) {
			return left.After(right)
		}
...
func isUnresolvedAmbiguousCodexRecoveryReason(reason string) bool {
	reason = strings.TrimSpace(reason)
	return strings.HasPrefix(reason, recoveryReasonAmbiguousCodexExecutionPrefix) ||
		reason == recoveryReasonAmbiguousAfterHelperRestart ||
		reason == recoveryReasonAmbiguousAfterHelperRestartNoticeSent ||
		reason == recoveryReasonHelperContextCanceledUnverified
}

func (b *Bridge) blockAutomaticTranscriptSyncForAmbiguousExecution(ctx context.Context, session Session, sourcePath string, checkpoint teamstore.ImportCheckpoint, turn teamstore.Turn) error {
	if err := b.markTranscriptImportBlocked(ctx, session, sourcePath, checkpoint); err != nil {
		return err
	}
	body := "Local Codex history sync is paused because a previous Codex execution is still unconfirmed. I did not publish its new transcript records as separate answers, and I did not retry automatically because that could duplicate work.\n\nAfter checking the existing Codex work, send `helper publish-history` here to import the pending history."
	body += "\n\n" + executionAnchorDiagnostics(session, sourcePath, checkpoint, turn)
	seed := firstNonEmptyString(turn.ID, checkpoint.LastRecordID, "ambiguous-execution")
	outboxID := "outbox:sync:" + session.ID + ":ambiguous-execution:" + shortStableID(seed)
	queued, err := b.queueOutbox(ctx, teamstore.OutboxMessage{
		ID:               outboxID,
		SessionID:        session.ID,
		TurnID:           "sync:" + session.ID,
		TeamsChatID:      session.ChatID,
		Kind:             "sync-status-ambiguous-execution",
		Body:             body,
		MentionOwner:     true,
		NotificationKind: "needs_attention",
	})
	...
	// Send only this notice: pending transcript deliveries from the ambiguous
...
func (b *Bridge) blockAutomaticTranscriptSyncForUnresolvedContinuation(ctx context.Context, session Session, local codexhistory.Session, checkpoint teamstore.ImportCheckpoint) error {
	return b.persistHistoryOnlyContinuationQuarantine(ctx, session, local, checkpoint, Transcript{
		SourceName:             local.FilePath,
		UnresolvedContinuation: true,
	})
}

func (b *Bridge) persistHistoryOnlyContinuationQuarantine(ctx context.Context, session Session, local codexhistory.Session, checkpoint teamstore.ImportCheckpoint, transcript Transcript) error {
	quarantine := transcript.TranscriptQuarantine
	if quarantine == nil {
		quarantine = &teamstore.TranscriptQuarantine{
			Kind:              "unresolved_continuation",
			SourcePath:        firstNonEmptyString(local.FilePath, checkpoint.SourcePath),
			SourceFingerprint: checkpoint.SourceFingerprint,
		}
		if transcript.FinalBoundary != nil {
			quarantine.FrontierRecordID = transcript.FinalBoundary.ID
			quarantine.FrontierLine = transcript.FinalBoundary.Line
			quarantine.FrontierOffset = transcript.FinalBoundary.StartOffset
		}
	}
	// A physical observation may have advanced the checkpoint immediately before
	// this call. Use the durable winner as the optimistic snapshot so the
	// quarantine is attached to that new cursor instead of being discarded as a
	// stale write.
	if current, found, err := b.store.ImportCheckpoint(ctx, transcriptCheckpointID(session.ID)); err != nil {
		return err
	} else if found {
		checkpoint = current
	}
	return b.persistTranscriptQuarantine(ctx, session, local.FilePath, checkpoint, quarantine)
}
...
func (b *Bridge) persistTranscriptQuarantine(ctx context.Context, session Session, sourcePath string, previous teamstore.ImportCheckpoint, quarantine *teamstore.TranscriptQuarantine) error {
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(current teamstore.ImportCheckpoint, found bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		// This is a history-only disposition. Keep the checkpoint usable for
		// live admission; `blocked` here used to turn a safe transcript
		// ambiguity into a chat-wide gate and made the warning recur after
		// restart. Source rewrite and real runtime ownership have separate
		// durable predicates.
		if current.Status == "" || current.Status == importCheckpointStatusBlocked {
			current.Status = importCheckpointStatusComplete
		}
		current.CompletionPending = false
...
func executionAnchorDiagnostics(session Session, sourcePath string, checkpoint teamstore.ImportCheckpoint, turn teamstore.Turn) string {
	...
	cutoffRecordID := strings.TrimSpace(checkpoint.LastRecordID)
	cutoffLine := checkpoint.LastSourceLine
	cutoffOffset := checkpoint.LastOffset
	if anchor != nil {
		threadID = firstNonEmptyString(anchor.ThreadID, threadID)
		outerTurnID = firstNonEmptyString(anchor.OuterTurnID, outerTurnID)
		reason = firstNonEmptyString(anchor.Reason, reason)
		sourcePath = firstNonEmptyString(anchor.SourcePath, sourcePath)
		cutoffRecordID = firstNonEmptyString(anchor.CutoffRecordID, cutoffRecordID)
	...
	observed := "none"
	if anchor != nil && len(anchor.ObservedTaskIDs) > 0 {
		observed = strings.Join(anchor.ObservedTaskIDs, ", ")
	}
	return fmt.Sprintf(
		"Execution ownership diagnostic (no full transcript was published):\n- session: `%s`\n- thread: `%s`\n- outer turn: `%s`\n- transcript source: `%s`\n- cutoff: record `%s`, line %d, offset %d\n- observed internal task IDs: `%s`\n- unresolved reason: `%s`\n- app-server cancellation fence: unconfirmed; no automatic retry",
		strings.TrimSpace(session.ID), threadID, outerTurnID, strings.TrimSpace(sourcePath), cutoffRecordID, cutoffLine, cutoffOffset, observed, reason,
	)
}
...
func (b *Bridge) blockAutomaticTranscriptSync(ctx context.Context, session Session, sourcePath string, checkpoint teamstore.ImportCheckpoint, backlogRecords int) error {
	// A bounded automatic scan is a background delivery policy, not a reason to
	// interrupt the Teams request path. Persist the trusted boundary and let the
	// next background poll or an explicit publish-history command decide how to
	// consume the backlog. Older versions posted a needs_attention message here,
	// which turned a normal backlog into a notification storm after restart.
	_ = backlogRecords
	if err := b.markTranscriptImportBlocked(ctx, session, sourcePath, checkpoint); err != nil {
		return err
	}
...
// retireLegacyHistoryGateOutbox removes only obsolete automatic history-policy
// notices emitted by versions that incorrectly made history delivery a user
// turn gate. The listener calls this before it starts sending. Queued and
// lease-expired Sending rows are safe to retire; a fresh Sending row remains
// recoverable until its ambiguous Graph request is no longer in flight. It
// intentionally does not inspect or mutate accepted/sent rows, and it leaves
// execution-ownership warnings alone.
func (b *Bridge) retireLegacyHistoryGateOutbox(ctx context.Context) (int, error) {
	if b == nil || b.store == nil {
		return 0, nil
	}
	return b.store.RetireLegacyHistoryGateOutboxForStartup(ctx, 128, 16)
}
...
func transcriptRecordIsKnownLiveStatusMirrorCandidate(record TranscriptRecord) bool {
	...
	// A live forwarder can persist its final fragment as an agent_message with
	// phase=final_answer while the same text has already been sent as progress.
	// The durable status hash is checked by shouldSkipKnownTranscriptOutboxRecord;
	// accepting this source shape there prevents the fragment from becoming an
	// anonymous duplicate of the later canonical final.
	if strings.EqualFold(strings.TrimSpace(record.Phase), "final_answer") {
		return strings.EqualFold(strings.TrimSpace(record.SourceType), "agent_message") ||
			strings.EqualFold(strings.TrimSpace(record.SourceType), "agentmessage")
	}
...
func addTranscriptReadProofToQueueOptions(proof *outboxQueueOptions, transcript Transcript) {
	if proof == nil || !transcript.SourceReadProofRangeKnown || strings.TrimSpace(transcript.SourceReadProofFingerprint) == "" {
		return
	}
	if transcript.SourceReadProofStartOffset < 0 || transcript.SourceReadProofEndOffset < transcript.SourceReadProofStartOffset {
		return
	}
	proof.ExpectedSourceReadFingerprint = strings.TrimSpace(transcript.SourceReadProofFingerprint)
	proof.ExpectedSourceReadStartOffset = transcript.SourceReadProofStartOffset
	proof.ExpectedSourceReadEndOffset = transcript.SourceReadProofEndOffset
	proof.ExpectedSourceReadRangeKnown = true
}

// applyTranscriptSourceProofToOutbox copies the complete automatic-import
// proof onto delivery rows that bypass the generic outbox planner. Large
// records and HTML batches use QueueTranscriptDeliveryOutbox directly, so
// relying on the planner's proof projection would leave those rows with only
// the cursor proof and allow a same-size tail rewrite to pass the sender
// fence. This helper is cold-path transcript work and intentionally does
// nothing for explicit history or legacy callers that did not supply a proof.
func applyTranscriptSourceProofToOutbox(msg *teamstore.OutboxMessage, proof outboxQueueOptions) {
	if msg == nil || strings.TrimSpace(proof.ExpectedSourcePath) == "" {
		return
	}
	if strings.TrimSpace(proof.ExpectedSourceFingerprint) != "" && proof.ExpectedSourceOffsetKnown && proof.ExpectedSourceOffset >= 0 {
		msg.TranscriptSourcePath = strings.TrimSpace(proof.ExpectedSourcePath)
		msg.TranscriptSourceProofFingerprint = strings.TrimSpace(proof.ExpectedSourceFingerprint)
		msg.TranscriptSourceProofOffset = proof.ExpectedSourceOffset
		msg.TranscriptSourceProofOffsetKnown = true
	}
	if strings.TrimSpace(proof.ExpectedSourceReadFingerprint) != "" && proof.ExpectedSourceReadRangeKnown &&
		proof.ExpectedSourceReadStartOffset >= 0 && proof.ExpectedSourceReadEndOffset >= proof.ExpectedSourceReadStartOffset {
		msg.TranscriptSourcePath = strings.TrimSpace(proof.ExpectedSourcePath)
		msg.TranscriptSourceReadProofFingerprint = strings.TrimSpace(proof.ExpectedSourceReadFingerprint)
		msg.TranscriptSourceReadProofStartOffset = proof.ExpectedSourceReadStartOffset
		msg.TranscriptSourceReadProofEndOffset = proof.ExpectedSourceReadEndOffset
		msg.TranscriptSourceReadProofRangeKnown = true
	}
}

// transcriptAutomaticImportProofQueueOptions binds every automatic history
// import row to the source prefix that was observed before the read began.
// Existing checkpoints use their durable proof.  A first import has no
// checkpoint yet, so its proof is the bounded fingerprint of the complete
// source prefix observed at read start.  This helper is on the cold import
// path; ordinary poll/admission paths do not pay for it.
func transcriptAutomaticImportProofQueueOptions(sourcePath string, checkpoint teamstore.ImportCheckpoint, sourceSizeBeforeRead int64) outboxQueueOptions {
	if checkpoint.SourceRewriteBlocked {
		return outboxQueueOptions{}
	}
	...
	if strings.TrimSpace(proof.ExpectedSourceFingerprint) != "" {
		start := int64(0)
		if linkedCheckpointOffsetKnown(checkpoint) && checkpoint.LastOffset >= 0 {
			start = checkpoint.LastOffset
		}
	...
	// old/unverifiable boundary. Do not manufacture a proof from the current
	// file (especially not from its current EOF): that would turn a replaced
	// rc16-era transcript into an apparently safe suffix and silently skip or
	// duplicate history. Explicit publish-history is the recovery boundary for
	// this shape; automatic/background import stays paused and silent.
	if strings.TrimSpace(checkpoint.LastRecordID) != "" {
		return outboxQueueOptions{}
	}
	sourcePath = strings.TrimSpace(sourcePath)
...
// from queueing even its title when the existing checkpoint cannot be bound to
// the current source. This is deliberately cold-path work; the ordinary Teams
// admission and outbox paths do not call it.
func (b *Bridge) preflightAutomaticTranscriptImportSourceProof(ctx context.Context, session Session, sourcePath string, checkpointID string) (outboxQueueOptions, error) {
	if b == nil || b.store == nil {
		return outboxQueueOptions{}, nil
	}
	checkpoint, found, err := b.store.ImportCheckpoint(ctx, checkpointID)
	if err != nil {
	...
	proof := transcriptAutomaticImportProofQueueOptions(sourcePath, checkpoint, sourceSize)
	if strings.TrimSpace(proof.ExpectedSourceFingerprint) != "" &&
		transcriptAutomaticImportSourceProofMatches(proof) {
		return proof, nil
	}
	if strings.TrimSpace(proof.ExpectedSourceFingerprint) == "" {
		if err := b.blockAutomaticTranscriptSync(ctx, session, sourcePath, checkpoint, 0); err != nil {
			return outboxQueueOptions{}, err
		}
		return outboxQueueOptions{}, errTranscriptAutomaticSourceProofUnavailable
	}
...
func (b *Bridge) recordTranscriptCheckpointProgressDetailedWithID(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string) error {
	return b.recordTranscriptCheckpointProgressDetailedWithSourceProof(ctx, session, sourcePath, lastRecordID, lastLine, lastOffset, checkpointID, outboxQueueOptions{})
}
...
func (b *Bridge) recordTranscriptCheckpointProgressDetailedWithParentFence(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, parentFenceSessionID string) error {
	return b.recordTranscriptCheckpointProgressDetailedWithSourceProofAndParentFence(ctx, session, sourcePath, lastRecordID, lastLine, lastOffset, checkpointID, outboxQueueOptions{}, parentFenceSessionID)
}

func (b *Bridge) recordTranscriptCheckpointProgressDetailedWithSourceProof(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, sourceProof outboxQueueOptions) error {
...
func (b *Bridge) recordTranscriptCheckpointProgressDetailedWithSourceProofAndParentFence(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, sourceProof outboxQueueOptions, parentFenceSessionID string, finalBoundary ...TranscriptFinalBoundary) error {
	return b.recordTranscriptCheckpointObservation(ctx, session, sourcePath, lastRecordID, lastLine, lastOffset, checkpointID, sourceProof, parentFenceSessionID, nil, finalBoundary...)
}

// transcriptCheckpointObservation is the narrow extension used when a scan
// has physically consumed a suffix but must retain a bounded semantic history
// range. Ordinary visible progress keeps the legacy helper above; gap progress
// uses this path so cursor and disposition are committed by one checkpoint CAS.
type transcriptCheckpointObservation struct {
	// ReplaceSemanticState distinguishes a scanner snapshot that explicitly
	// observed the current semantic state (including an intentional nil/clear)
	// from legacy callers that only want to advance the physical cursor.
	ReplaceSemanticState bool
	// ExpectedCheckpoint is the scan's optimistic snapshot.  The bridge writer
	// compares it inside the store CAS so a slow reader cannot overwrite a newer
...
func transcriptCheckpointObservationForTranscript(transcript Transcript, expected *teamstore.ImportCheckpoint, expectedFound bool) *transcriptCheckpointObservation {
	...
	if expected != nil {
		snapshot := *expected
		observation.ExpectedCheckpoint = &snapshot
		observation.ExpectedCheckpointFound = expectedFound
	}
	return observation
}

func (b *Bridge) recordTranscriptCheckpointObservation(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, sourceProof outboxQueueOptions, parentFenceSessionID string, observation *transcriptCheckpointObservation, finalBoundary ...TranscriptFinalBoundary) error {
	...
	_, _, err := updateCheckpoint(checkpointID, func(previous teamstore.ImportCheckpoint, found bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		if observation != nil && observation.ExpectedCheckpoint != nil {
			expected := *observation.ExpectedCheckpoint
			expectedCursorUsable := linkedCheckpointOffsetKnown(expected) || (!expected.LastOffsetKnown && expected.LastOffset == 0)
			sameSource := found && observation.ExpectedCheckpointFound &&
				cleanComparablePath(previous.SourcePath) == cleanComparablePath(expected.SourcePath) &&
				(strings.TrimSpace(expected.SourceGeneration) == "" || strings.TrimSpace(previous.SourceGeneration) == strings.TrimSpace(expected.SourceGeneration)) &&
				linkedCheckpointOffsetKnown(previous) && expectedCursorUsable &&
				previous.LastOffset >= expected.LastOffset
			if sameSource && previous.LastOffset >= lastOffset && previous.LastOffset > expected.LastOffset {
				// A competing reader already committed this or a later physical
				// cursor for the same source. Never roll it back; the current
				// reader's outbox dedupe/ledger path will make its stale prefix a
				// no-op.
				return previous, false, nil
			}
			if found != observation.ExpectedCheckpointFound ||
				(found && !sameSource) ||
				(found && sameSource && previous.LastOffset == lastOffset && !importCheckpointSameExceptUpdatedAt(previous, expected)) {
				return previous, false, teamstore.ErrTranscriptObservationConflict
			}
		}
		if strings.TrimSpace(sourceProof.ExpectedSourceFingerprint) != "" &&
			!transcriptSourceProofQueueOptionsMatches(sourceProof) {
			// The outbox may already be queued, but its own sender-side proof will
			// keep it from Graph. Do not advance the cursor over a source rewrite
			// that happened between queue and checkpoint.
			return previous, false, errTranscriptAutomaticSourceProofUnavailable
		}
		// This progress API is also used by active-turn and recovery readers,
		// which may have scanned the source outside the store transaction.  Before
		// advancing a cursor, revalidate the previously trusted prefix while the
		// current checkpoint is still available.  Otherwise a replacement between
		// the scan and this write could manufacture a fresh fingerprint and move
		// the cursor across bytes that were never proven to belong to this source.
		if !transcriptImportRunIsExplicitHistory(previous.ImportTurnID) && strings.TrimSpace(previous.LastRecordID) != "" {
			if linkedTranscriptCheckpointNeedsAutomaticSourceProof(previous, sourcePath) || !linkedCheckpointPrefixMatches(sourcePath, previous) {
				return previous, false, teamstore.ErrUnresolvedExecution
			}
		}
		if executionAnchorActive(previous.UnresolvedExecution) &&
			!strings.HasPrefix(strings.TrimSpace(previous.ImportTurnID), "publish-history:") &&
			!strings.HasPrefix(strings.TrimSpace(previous.ImportTurnID), "publish-full:") {
			// The final checkpoint write is the last ownership barrier after
		...
		if observation == nil && previous.TranscriptQuarantine != nil && !historyTieredQuarantineIsContextGap(previous.TranscriptQuarantine) && !transcriptImportRunIsExplicitHistory(previous.ImportTurnID) &&
		...
		if observation != nil {
			...
			if observation.ReplaceSemanticState {
				next.TerminalBoundarySeen = observation.TerminalBoundary != nil
				if observation.TerminalBoundary != nil {
	...
	if errors.Is(err, errTranscriptAutomaticSourceProofUnavailable) {
		if blockErr := b.blockAutomaticTranscriptSyncForSourceRewrite(ctx, session, sourcePath, teamstore.ImportCheckpoint{
			ID: checkpointID, SessionID: session.ID, SourcePath: sourcePath,
		}); blockErr != nil {
			return blockErr
		}
	}
...
func (b *Bridge) recordTranscriptLedgerAfterCheckpointProgress(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, parentFenceSessionID string) error {
	...
	if checkpoint.TranscriptQuarantine != nil && !historyTieredQuarantineIsContextGap(checkpoint.TranscriptQuarantine) && !transcriptImportRunIsExplicitHistory(checkpoint.ImportTurnID) {
...
func transcriptCheckpointProgressAtOrAhead(previous teamstore.ImportCheckpoint, sourcePath string, lastLine int, lastOffset int64) bool {
	if previous.Status == importCheckpointStatusImporting {
		return false
	}
	if cleanComparablePath(previous.SourcePath) == "" || cleanComparablePath(sourcePath) == "" || cleanComparablePath(previous.SourcePath) != cleanComparablePath(sourcePath) {
		return false
	...
	if previous.LastOffset > 0 && lastOffset > 0 {
		return previous.LastOffset > lastOffset || (previous.LastOffset == lastOffset && previous.LastSourceLine >= lastLine)
	}
	return previous.LastOffset == 0 && lastOffset == 0 && previous.LastSourceLine >= lastLine
}
...
// visible to the next watcher pass.  A checkpoint may be recorded immediately
// after the last complete record while app-server is still writing the next
// line; persisting the whole file size in that window makes the unchanged-file
// fast path skip the completed tail forever.  The common complete-file path is
...
// markTranscriptImportCompleteFromResult closes an import at the last record
// actually read. It advances to EOF only when the source is unchanged since
// the read snapshot; an append that races the import is intentionally left for
// the next pass instead of being skipped by an optimistic EOF cursor.
func (b *Bridge) markTranscriptImportCompleteFromResult(ctx context.Context, session Session, sourcePath string, result transcriptImportResult, checkpointID string) error {
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(checkpoint teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		sourceRewritten := cleanComparablePath(checkpoint.SourcePath) == cleanComparablePath(sourcePath) &&
			((sourceSize < checkpoint.SourceSize) ||
				(!checkpoint.SourceModTime.IsZero() && !sourceModTime.IsZero() && !checkpoint.SourceModTime.Equal(sourceModTime) && sourceSize <= checkpoint.SourceSize))
		if !sourceRewritten && transcriptCheckpointProgressAtOrAhead(checkpoint, sourcePath, result.LastLine, result.LastOffset) {
			return checkpoint, false, nil
		}
		checkpoint.ID = checkpointID
		checkpoint.SessionID = session.ID
		checkpoint.SourcePath = sourcePath
		if strings.TrimSpace(result.LastRecordID) != "" {
			checkpoint.LastRecordID = result.LastRecordID
			checkpoint.LastSourceLine = result.LastLine
		}
		if result.LastOffset > 0 {
			checkpoint.LastOffset = result.LastOffset
		}
		if strings.TrimSpace(result.LastRecordID) != "" {
			checkpoint.LastOffsetKnown = true
		}
		...
		checkpoint.PendingHistoryRange = nil
		checkpoint.ContextGap = nil
...
// markTranscriptImportCompleteAtEOFWithSourceProof closes the automatic EOF
// path with the same source proof that was captured before scanning. The
// ordinary detailed helper recomputes a fingerprint from the current pathname;
// that is unsafe if a same-size replacement happens between the preflight and
// the checkpoint update. Keeping the expected proof in the transaction makes
// the checkpoint conservative even when the source changes during finalization.
func (b *Bridge) markTranscriptImportCompleteAtEOFWithSourceProof(ctx context.Context, session Session, sourcePath string, result transcriptImportResult, checkpointID string) error {
	if strings.TrimSpace(checkpointID) == "" {
		checkpointID = transcriptCheckpointID(session.ID)
	}
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(checkpoint teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		// source rewrite.  The size/mtime and prefix CAS above prove that this is
		// still the same snapshot; persist a proof for the new EOF boundary.
		checkpoint.SourceFingerprint = strings.TrimSpace(transcriptCheckpointSourceFingerprint(sourcePath, sourceSize))
		if checkpoint.SourceFingerprint == "" {
			return checkpoint, false, errTranscriptAutomaticSourceProofUnavailable
		}
		if explicitHistory {
			// An explicit history publication is the operator-selected recovery
...
// markTranscriptImportCompletionPending records that the transcript scan and
// its durable cursor reached the completion phase, but the subagent marker and
// final import notice still need to be made idempotently. Keeping this bit
// separate from Status lets the same owner retry a failed notification without
// treating a fully scanned transcript as a fresh source boundary.
func (b *Bridge) markTranscriptImportCompletionPending(ctx context.Context, session Session, sourcePath string, checkpointID string) error {
	if b == nil || b.store == nil {
		return nil
	}
	checkpointID = strings.TrimSpace(firstNonEmptyString(checkpointID, transcriptCheckpointID(session.ID)))
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(checkpoint teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		checkpoint.Status = importCheckpointStatusImporting
		checkpoint.CompletionPending = true
		checkpoint.UpdatedAt = now
		return checkpoint, true, nil
	})
	return err
...
func (b *Bridge) markTranscriptImportPausedAt(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, importTurnID string, kindPrefix string) error {
	return b.markTranscriptImportPausedAtWithProof(ctx, session, sourcePath, lastRecordID, lastLine, lastOffset, checkpointID, importTurnID, kindPrefix, outboxQueueOptions{})
}

// markTranscriptImportPausedAtWithProof keeps a budgeted automatic import
// bound to the source snapshot that produced its records. The old helper
// recomputed the checkpoint fingerprint from the current pathname at pause
// time; a replacement between the scan and this write could therefore bind an
// old LastRecordID to a new file. Explicit history has no automatic proof and
// keeps the compatibility wrapper above.
func (b *Bridge) markTranscriptImportPausedAtWithProof(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, lastOffset int64, checkpointID string, importTurnID string, kindPrefix string, sourceProof outboxQueueOptions) error {
	if strings.TrimSpace(checkpointID) == "" {
		checkpointID = transcriptCheckpointID(session.ID)
	}
	if strings.TrimSpace(sourceProof.ExpectedSourceFingerprint) != "" &&
		...
		// A source race at the pause boundary is a silent automatic recovery
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(checkpoint teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		checkpoint.ID = checkpointID
		checkpoint.SessionID = session.ID
		// A paused scan can have no record before the incomplete or ambiguous
		// marker.  In that case lastRecordID/lastLine/lastOffset are zero values,
		// not a request to rewind the durable cursor.  Rewinding here would make
		// the next automatic pass take the full-transcript parser path and could
		// publish the quarantined child answer.  Keep the previous source and
		// position until a complete record actually advances them; initialize the
		// source metadata only for a brand-new checkpoint.
		progressed := strings.TrimSpace(lastRecordID) != "" || lastLine > 0 || lastOffset > 0
		if progressed {
			checkpoint.SourcePath = sourcePath
...
func (b *Bridge) markTranscriptImportCompleteDetailedWithID(ctx context.Context, session Session, sourcePath string, lastRecordID string, lastLine int, checkpointID string, completeEOF bool) error {
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, checkpointID, func(checkpoint teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		checkpoint.SessionID = session.ID
		checkpoint.SourcePath = sourcePath
		preserveCurrentPosition := completeEOF &&
...
func transcriptImportRunAllowsAmbiguous(currentTurnID string, requestedTurnID string) bool {
	for _, value := range []string{currentTurnID, requestedTurnID} {
		value = strings.TrimSpace(value)
		if strings.HasPrefix(value, "publish-history:") || strings.HasPrefix(value, "publish-full:") {
			return true
		}
	}
	return false
...
func transcriptImportRunIsAutomatic(turnID string) bool {
	turnID = strings.TrimSpace(turnID)
	return strings.HasPrefix(turnID, "import:") || strings.HasPrefix(turnID, "import-bg:")
}

func transcriptOutboxAutomaticRun(turnID string) bool {
	turnID = strings.TrimSpace(turnID)
	return strings.HasPrefix(turnID, "sync:") || transcriptImportRunIsAutomatic(turnID)
}

func transcriptImportRunIsExplicitHistory(turnID string) bool {
	turnID = strings.TrimSpace(turnID)
	return strings.HasPrefix(turnID, "publish-history:") || strings.HasPrefix(turnID, "publish-full:")
}

func transcriptCheckpointAdvanceBlocked(checkpoint teamstore.ImportCheckpoint) bool {
	anchor := checkpoint.UnresolvedExecution
	if anchor == nil || strings.TrimSpace(anchor.State) == "resolved" {
		return false
	}
	return !transcriptImportRunAllowsAmbiguous(checkpoint.ImportTurnID, "")
}
...
func (b *Bridge) markTranscriptImportBlocked(ctx context.Context, session Session, sourcePath string, previous teamstore.ImportCheckpoint) error {
	return b.markTranscriptImportBlockedWithOptions(ctx, session, sourcePath, previous, false)
}

func (b *Bridge) markTranscriptImportBlockedForSourceRewrite(ctx context.Context, session Session, sourcePath string, previous teamstore.ImportCheckpoint) error {
	return b.markTranscriptImportBlockedWithOptions(ctx, session, sourcePath, previous, true)
}
...
func (b *Bridge) markTranscriptImportBlockedWithOptions(ctx context.Context, session Session, sourcePath string, previous teamstore.ImportCheckpoint, sourceRewrite bool) error {
	...
	_, _, err := b.store.UpdateImportCheckpoint(ctx, id, func(checkpoint teamstore.ImportCheckpoint, _ bool, now time.Time) (teamstore.ImportCheckpoint, bool, error) {
		...
		if before.Status == importCheckpointStatusBlocked && importCheckpointSameExceptUpdatedAt(before, checkpoint) {
			return checkpoint, false, nil
		}
		checkpoint.UpdatedAt = now
		return checkpoint, true, nil
