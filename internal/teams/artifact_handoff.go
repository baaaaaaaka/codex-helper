...
- Add --adapter-shell user when scheduler submission depends on the user's normal shell setup, such as modules, submit_job, NSS, or SUBMIT_ACCOUNT; cxp resolves that shell environment before direct-running the adapter
...
func defaultControlFallbackHelpDigest() string {
	return strings.Join([]string{
		...
		"`cxp beacon profile create <name> ... --adapter-shell user` - resolve the user's shell setup before direct-running the adapter",
