...
func teamsControlFallbackHelpContext() string {
	lines := []string{
		...
		"- If worker doctor reports `missing codex`, set scheduler PATH or pass worker `--codex-path <codex-or-wrapper>` from the adapter. Use a worker wrapper for Codex exec flags such as `--skip-git-repo-check`; Teams service `--codex-arg` does not automatically reach remote beacon workers. Generated templates honor `CXP_BEACON_CODEX_BIN` as worker `--codex-path`.",
