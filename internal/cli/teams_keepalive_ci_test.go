...
func TestTeamsBackgroundKeepaliveWSLBootstrapSuccessCleansStartupFallbackCI(t *testing.T) {
	...
	for _, want := range []string{
		...
		"WSL Startup fallback process discovery failed",
		"WSL Startup fallback files remain after cleanup",
		"WSL Startup fallback processes remain after cleanup",
		"WSL Startup fallback process verification failed",
		"if (Test-Path -LiteralPath $path -ErrorAction Stop) { Remove-Item",
